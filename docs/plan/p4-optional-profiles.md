...
pass. This closes the downstream CDP fencing component plus real-backend
...
validation. Historical provisioning lock `fb1b2b0` records that
process/component baseline.

Runner implementation `a2b82b0` and focused corrections through harness
`550c785` compose the complete local ADR 0033 topology. Clean run
`20260906T050213.016063000Z` passes 13/13 on `linux/arm64` through two Gateway
processes, two independent mTLS/JWS caller processes, one authenticated unique
ingress, retained Valkey, and signed real Chromium. The Contract/tree/48-case
identity is pinned and six Provider routes are exercised, but the Suite is not
(`suite_exercised=false`). Exactly five `0600` files pass all cleanup and
sanitization checks. Hosted run `34013982796` at checkout/harness `2cadc53`
independently passes the same 13 scenarios on `linux/amd64`. Downloaded artifact
...
pins the expected amd64 identities, topology, cleanup, and sanitization state.
The GitHub artifact has digest
`sha256:9c00f3ba184e82d7eff661b831c05c1bdf331fa41caf2d8bbb3353168ab155b0`.
These close only the named ADR 0033 caller gates on their platforms.

Hosted checkout `2cadc53` also passes repository CI `34013982778`, Reference
`34013982794`, Candidate `34013982784`, Browser `34013982785`, shared capacity
`34013982786`, and durable revocation `34013982798`; these remain separate
evidence tracks.

ADR 0034 now adds an explicitly selected v2 action-fencing component without
changing the locked ADR 0033 v1 identities or callers. One permanent bounded
Redis hash retains canonical fingerprint-only session history and checkpoint
state; an independent sequence/token witness rejects a missing or restored-old
hash and durably confirms each first or higher-fence activation. The included
Darwin/Linux file witness is single-process evidence and supplies independence
only outside the Redis snapshot/restore domain. Focused/full race-shuffle, vet,
Contract verification, the unchanged 48-case Suite, and tagged integration
against the pinned Valkey index pass. A separately locked 18-scenario v2 caller
...
orchestrator fault credential. It has no clean local or hosted result yet, and
adds no v2 deletion/restore E2E, production witness, HA/failover, deployment,
advertisement, or production claim.
...
- ADR 0033 implementation `b4d41c9` passes targeted/full race-shuffle, vet,
  ...
  adapter, private ingress component, and Browser composition close the
  component plus real-backend adapter integration gate.
- ADR 0034's separately selected v2 component passes focused/full
  race-shuffle, vet, Contract verification, the unchanged 48-case Suite, and
  ...
  monotonic witness. A separate v2 downstream caller stack is implemented but
  has not yet produced a clean local or hosted run.
- Implementation `58488d7` proves two independent downstream caller processes
  ...
  fail-closed parent process manager. These remain process/component evidence.
- Harness `550c785` run `20260906T050213.016063000Z` passes all 13 locked
  scenarios on local `linux/arm64` through two Gateways, two independent
  callers, one authenticated unique ingress, retained Valkey, and signed real
...
- Hosted checkout/harness `2cadc53` passes repository CI `34013982778` plus
  Reference/Candidate/Browser/shared-capacity/durable-revocation/downstream
  runs `34013982794`/`34013982784`/`34013982785`/`34013982786`/
  `34013982798`/`34013982796`. The last run independently passes the ADR 0033
  13-case gate on `linux/amd64`; its inspected artifact digest is
  `sha256:9c00f3ba184e82d7eff661b831c05c1bdf331fa41caf2d8bbb3353168ab155b0`.
  The six E2E profiles remain separate and must not be composed into an
  aggregate claim.
...
- ADR 0033 and `b4d41c9` pass targeted/full race-shuffle, vet, Contract
  ...
  `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`.
  This establishes the downstream action-fence ports, Redis-compatible
  exact-member/high-water adapter, private ingress component, and fail-closed
  Browser composition as component plus real-backend adapter integration
  evidence.
- ADR 0034 witnessed-v2 component validation passes focused/full race-shuffle,
  vet, Contract verification, the unchanged locked 48-case Suite, and tagged
  integration against the same pinned Valkey index. This is deletion and
...
- Downstream caller bootstrap `58488d7`, provisioning/process implementation
  ...
  acceptance and FIFO type is not proof of an anonymous pipe.
- Runner implementation `a2b82b0` and corrections through harness `550c785`
  compose the complete local topology. Run `20260906T050213.016063000Z` passes
  all 13 scenarios on `linux/arm64`. Contract identity is pinned and six
  protected Provider routes are exercised, but the Suite is not
  (`suite_exercised=false`). The exact five-file `0600` evidence set passes all
  cleanup and sanitization checks. Hosted checkout/harness `2cadc53` run
  `34013982796` passes the same 13 scenarios on `linux/amd64`. Downloaded GitHub
  artifact `browser-downstream-fencing-e2e-evidence-34013982796` contains
  evidence directory `20260906T052710.781616339Z` with exactly five sanitized
...
ADR 0032 passes its exact-grant revocation port and real Redis-compatible
adapter component gate plus its separately locked local and hosted
two-Gateway/independent-revoker caller gate. ADR 0033/`b4d41c9` passes its
downstream CDP fencing component and real-backend adapter integration gate;
harness/run `550c785`/`20260906T050213.016063000Z` and
...
adapter gates; its separately locked v2 caller runner is implemented but has no
recorded clean local or hosted result.

Keep `suite_exercised=false` for both downstream caller profiles until a runner
actually invokes the Contract Suite. Next commit, execute, and inspect the
locked v2 real-Chromium caller gate covering retained-history deletion,
controlled older-snapshot restore, witness reconstruction, and no upstream
dial/old action. Keep Valkey
provenance/HA/failover, a production witness and restore procedure, metrics,
