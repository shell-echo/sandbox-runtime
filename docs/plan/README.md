...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), Provider-local components, default-disabled command/runtime graph (`66183b1`), process-local Gateway limits, hosted Browser Reference E2E 13+5 run `33970773330`, Redis-compatible shared capacity `9434540`, local and hosted two-Gateway shared-capacity evidence, ADR 0032 component plus local/hosted caller evidence, and ADR 0033/`b4d41c9` component evidence pass their named gates. Harness `550c785` local run `20260906T050213.016063000Z` additionally passes all 13 two-Gateway/unique-ingress/retained-Valkey/signed-real-Chromium scenarios on `linux/arm64`; its Contract Suite is not executed. Hosted downstream repetition, deleted/restore-missing high-water controls, restored-snapshot consistency, Valkey provenance/HA/failover, production storage/configuration/metrics/deployment, production advertisement/public Gateway, aggregate, multi-controller, multi-tenant, and production gates remain open |
