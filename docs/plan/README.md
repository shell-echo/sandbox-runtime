...
| P2.5 | [Coding/shell vertical composition](p2.5-coding-shell-vertical-composition.md) | P2.5a-e pass through CI `32937530059`; P2.5f0-f7 retain their recorded local/CI gates; P2.5g implementation `0e6e108` and CI `33157119149` pass; P2.5h implementation `2c55173` and CI `33159099578` pass; current co-located P2.5i reference caller lock `58ed009`/`c91d83f` passed locally and in hosted run `33724009124` with 15+5 scenarios; real Agent Platform E2E, aggregate conformance, reliability, tenancy, deployment, and production gates remain open |
...
| P3 | [Migration readiness](p3-migration-readiness.md) | Local binding/shadow/metrics component evidence plus current local and Hosted candidate integration (`58ed009`/`c91d83f`, 15+5 scenarios; run `33724009180`) exist; real platform shadow E2E, canary, rollback, drain, and migration gates remain blocked |
| P3 candidate | [Platform candidate caller](p3-platform-candidate-caller.md) | Candidate caller and Docker process harness passed locally and in hosted run `33724009180`; evidence remains distinct from real Veronica platform evidence |
...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), and uncomposed Provider-local session/reference/usage components have named evidence; runtime/transport/Gateway composition remains next |
