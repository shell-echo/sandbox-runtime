...
| P3 | [Migration readiness](p3-migration-readiness.md) | Local binding/shadow/metrics evidence, hosted candidate run `33955437046` against harness/Provider `c7fe24d`/`c0a55d1`, and current local candidate run `20260905T080623.861033000Z` against `59e08d5`/`c0a55d1` exist; real platform shadow E2E, canary, rollback, drain, and migration gates remain blocked |
| P3 candidate | [Platform candidate caller](p3-platform-candidate-caller.md) | Candidate caller and Docker process harness passed in hosted run `33955437046` against `c7fe24d`/`c0a55d1` and current local run `20260905T080623.861033000Z` against `59e08d5`/`c0a55d1`; evidence remains distinct from real Veronica platform evidence |
...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), Provider-local components, default-disabled command/runtime graph (`66183b1`), process-local Gateway limits, hosted Browser Reference E2E 13+5 run `33955436984`, Redis-compatible shared capacity `9434540`, local arm64 and hosted amd64 two-Gateway shared-capacity evidence including current-lock run `33955436968`, ADR 0032 exact-grant revocation port/Redis-compatible adapter component `c0a55d1`, and local plus hosted seven-scenario two-Gateway/independent-revoker evidence under harness `e952ef9`/run `33959122456` pass their named gates; downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, production advertisement/public Gateway, aggregate, multi-controller, multi-tenant, deployment, and production gates remain open |
