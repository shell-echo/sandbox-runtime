...
| P3 | [Migration readiness](p3-migration-readiness.md) | Local binding/shadow/metrics component evidence plus latest local and hosted candidate evidence `e1de512`/`cd33ba3` (15+5 scenarios; hosted run `33732133569`) exist; real platform shadow E2E, canary, rollback, drain, and migration gates remain blocked |
| P3 candidate | [Platform candidate caller](p3-platform-candidate-caller.md) | Candidate caller and Docker process harness passed locally and in hosted run `33732133569`; evidence remains distinct from real Veronica platform evidence |
...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), uncomposed session/reference/usage components, and a fail-closed Docker adapter/private-relay component have named evidence; real restricted-egress/provenance dependencies, protected transport, Gateway, advertisement, and Browser caller remain open |
