...
| P2.5 | [Coding/shell vertical composition](p2.5-coding-shell-vertical-composition.md) | P2.5a-h retain their recorded local/CI gates; hosted P2.5i run `33846603323` passed 15+5 scenarios against current harness/Provider `28a9a5e`/`7b062e6`, while the latest local run remains on earlier lock `9eb32ba`/`5aae281`; real Agent Platform E2E, aggregate conformance, reliability, tenancy, deployment, and production gates remain open |
...
| P3 | [Migration readiness](p3-migration-readiness.md) | Local binding/shadow/metrics evidence and hosted candidate run `33846603454` against current harness/Provider `28a9a5e`/`7b062e6` exist; the latest local candidate remains on earlier lock `9eb32ba`/`5aae281`; real platform shadow E2E, canary, rollback, drain, and migration gates remain blocked |
| P3 candidate | [Platform candidate caller](p3-platform-candidate-caller.md) | Candidate caller and Docker process harness passed in hosted run `33846603454` against current lock and locally against the earlier lock; evidence remains distinct from real Veronica platform evidence |
...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), Provider-local components, default-disabled command/runtime graph (`66183b1`), process-local Gateway capacity (`7b062e6`), and hosted Browser Reference E2E 11+5 run `33846603547` against harness/Provider `28a9a5e`/`7b062e6` pass their named gates; production advertisement/public Gateway, pre-upgrade edge controls, distributed capacity/revocation, aggregate, multi-controller, multi-tenant, deployment, and production gates remain open |
