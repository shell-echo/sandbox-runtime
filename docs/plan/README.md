...
| P4 Browser | [Optional Profiles](p4-optional-profiles.md) | Browser Contract/projection, exact sandboxed signed amd64/arm64/v8 image publication (`33724368530`), Provider-local components, default-disabled command/runtime graph (`66183b1`), process-local Gateway capacity (`7b062e6`), pre-upgrade service limits (`44ea2ee`), and hosted Browser Reference E2E 13+5 run `33857739150` against harness/Provider `7a20d9d`/`b8f8941` pass their named gates; production advertisement/public Gateway, authenticated partition-aware shared capacity, durable distributed revocation, production storage/configuration and metrics, aggregate, multi-controller, multi-tenant, deployment, and production gates remain open |
