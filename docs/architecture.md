...
| Terminal | P2.5f1 adds a backend-neutral terminal runtime, PTY-owning guest broker, and Docker adapter with private durable identity, bounded capacity, cleanup, and same-shell reattach after Provider driver reconstruction. It is not connected to session authority and has no WebSocket adapter. | Begin P2.5f2 with durable coordination, repository migration, and lifecycle-authority synchronization, then complete opaque resolution, bounded WebSocket, Gateway, and command composition through P2.5f7. |
