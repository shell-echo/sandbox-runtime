...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy `7e60340`; protected transport `b8423f5`; caller-owned Gateway `5aae281`; default-disabled command graph `66183b1`; DNS compatibility Provider `f760369`; Browser Gateway local capacity `7b062e6`; Browser pre-upgrade edge `44ea2ee`; Browser public TLS edge `b8f8941`; hosted E2E harness `7a20d9d` | Locks Browser authority, composes the default-disabled Provider-local graph, bounds total/per-session Gateway connections and pre-upgrade service work inside one process, and adds a bounded accepted-connection TLS 1.3/HTTP/1.1 listener with explicit header/time limits to the reference deployment. It adds no production advertisement or Provider-owned public Gateway. Hosted Browser Reference E2E is recorded separately; partition-aware shared capacity, distributed durable revocation, production storage/configuration and metrics, real Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production claims remain open |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; Browser caller harness `7a20d9d` | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, admission, default-disabled command composition, process-local Gateway capacity, pre-upgrade service limits, and hosted 13+5 listener/TLS/HTTP caller evidence have named evidence. Production advertisement/public Gateway, partition-aware shared capacity, distributed durable revocation, production storage/configuration and metrics, remaining profile-specific security/concurrency, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
| P4 browser Contract authority and reference caller | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; capacity `7b062e6`; service edge `44ea2ee`; TLS edge `b8f8941`; hosted harness/Provider `7a20d9d`/`b8f8941`; Browser run `33857739150` | Browser authority, DTO projection, image/runtime components, default-disabled command graph, process-local post-authorization capacity, pre-upgrade service limits, and hosted 13+5 listener/TLS/HTTP scenarios pass their named gates. Hosted artifact digest is `sha256:94bcdfa53b667d4a6bc17fd6714cc9895e8402b830b8c6425c604c835f9228f`. Partition-aware shared capacity, distributed durable revocation, production storage/configuration and metrics, production advertisement/public Gateway, real platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production evidence remain absent |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, exact sandboxed signed linux/amd64 plus linux/arm64/v8 publication, Provider-local components, default-disabled command graph, process-local Gateway capacity, pre-upgrade service limits, and hosted 13+5 listener/TLS/HTTP Browser caller evidence have named evidence | Close authenticated partition-aware shared capacity, distributed durable revocation, production storage/configuration and metrics, remaining hostile-tenant/operational, and deployment gates before any production advertisement; then begin the Desktop authority audit |
...
- Browser production advertisement and deployable public caller-owned Gateway,
  plus authenticated partition-aware shared or distributed capacity, durable
  distributed revocation, production storage/configuration and metrics,
