...
| --- | --- | --- |
| Latest local implementation evidence | P2.5f5 implementation `fdfc771` | A separate `gateway/composition` package composes caller-owned authorization, revocation, recording, and handshake admission with the existing Gateway, a Provider reference resolver, and the bounded f4 adapters. Focused and full race/shuffle, vet, module verification, unchanged Contract verification, the locked 38-case Suite, and diff checks passed. This is local Gateway composition evidence only; no command/configuration composition, repository CI, external caller, aggregate, multi-controller, multi-tenant, deployment, or readiness claim is made |
| Latest repository CI evidence | P2.5f4 implementation `14f14cc`; evidence baseline `1d9da67` | Repository CI `33064864447` passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `1d9da67`, which contains the P2.5f4 implementation. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f5 Gateway composition evidence | implementation `fdfc771` | `gateway/composition` accepts only caller-supplied `Authorizer`, `RevocationSource`, `Recorder`, and WebSocket handshake admission plus a Provider reference resolver; no allow-all fallback exists. It resolves fresh opaque references on every Gateway attempt and wraps every Provider terminal dial in the f4 bounded adapter. Tests cover missing and typed-nil dependencies, incomplete endpoint rejection, grant binding, caller cross-tenant denial, endpoint expiry, revocation interruption, reconnect and exhaustion, bidirectional real-WebSocket flow, and metadata-only audit. Full race/shuffle, vet, module/Contract verification, the locked 38-case Suite, and diff checks passed locally. No command/configuration composition, repository CI, external caller, aggregate, multi-controller, tenancy, deployment, or production evidence is claimed |
...
| P2.5f5 | Gateway composition | Local composition gate passed in implementation `fdfc771`; repository CI remains pending | Retain caller-owned policy and no-command-composition/no-external-caller boundary; P2.5f6 command configuration is next |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1-f4 pass locally and in CI, and P2.5f5 has local composition evidence. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f6-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root can now
  select the Provider Docker lifecycle and exec adapters, but terminal/Gateway
  and artifact/usage applications are not command-composed, and startup
  advertises no capability or runtime profile. P2.5f0 design, P2.5f1 runtime,
  and P2.5f2 durable coordination/migration local and repository CI evidence
  pass. P2.5f3 opaque reference persistence/resolution local and repository CI
  evidence also pass. P2.5f4 WebSocket and terminal stream adapter local and
  repository CI evidence pass. P2.5f5 Gateway composition local evidence also
  passes; command composition remains next;
