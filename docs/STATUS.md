...
| ADR 0034 witnessed action-history component | ADR 0034; witnessed v2 Redis adapter and Darwin/Linux file witness; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`; Contract `5096e71fb84fbec22aa3487a0e55a1b49602ab8b`; tree `859f76dc0e855a0c8abdbbb5648df100dabb4328` | Local focused/full race-shuffle, vet, Contract lock verification, unchanged 48-case Suite, and tagged pinned-Valkey package integration pass. V2 retains at most 4,096 fingerprint-only session records in one permanent hash, rejects missing/malformed history, and compares its sequence/token with an independent monotonic witness; only an exact Redis-ahead-one interrupted commit may reconcile forward. The file witness is single-process component evidence and requires a separate durability/restore domain. Existing ADR 0033 callers and E2E remain v1. No v2 caller/Chromium deletion or restore run, production witness/storage, Valkey provenance/HA/failover, ACL separation, production configuration/advertisement/deployment, aggregate, multi-controller, hostile multi-tenant, or production-readiness result is claimed |
...
| P4 Browser witnessed action-history component | ADR 0034; witnessed v2 implementation; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd` | The explicit v2 successor adds permanent bounded fingerprint-only history, strict whole-hash validation, an independent monotonic checkpoint witness, exact one-step interrupted-CAS recovery, and stable fail-closed errors. Focused/full race-shuffle, vet, Contract verification, the unchanged 48-case Suite, and tagged real-Valkey adapter integration pass. It does not rewrite v1 or alter default composition. The included file witness is local single-process evidence, not HA or production storage. No v2 external-caller, real-Chromium deletion/restore, production witness, operational restore, deployment, or production gate is closed |
...
- ADR 0034 adds an explicitly selected v2 successor without changing those v1
  ...
  component and real-backend adapter evidence only. It adds no v2 caller,
  Chromium deletion/restore, production witness, HA/failover, ACL, deployment,
  or production evidence.
...
- a locked v2 multi-process real-Chromium caller gate covering retained-history
  deletion, controlled older-snapshot restore, witness reconstruction, and no
  upstream dial/old action; Browser production advertisement and deployable
...
Next, compose and lock the ADR 0034 v2 multi-process real-Chromium caller gate,
including retained-history deletion, controlled older-snapshot restore,
independent witness reconstruction, and proof of no upstream dial or old action.
