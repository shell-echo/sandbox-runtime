...
- real Browser restricted-egress provisioning, create-time policy binding,
  complete startup with the provenance verifier, protected transport, and
  Gateway composition;
...
and repository CI `33737531693`, including its `browser-provenance` job. The
verifier is not wired into a complete Browser startup, and the real
restricted-egress implementation is still absent; the adapter's live
`network=none` case is transport component evidence only. Create-policy binding,
protected/Gateway composition, handler routes, advertisement, Browser caller,
