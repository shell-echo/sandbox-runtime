...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy implementation `7e60340`; Browser protected transport `b8423f5`; caller-owned Browser Gateway `5aae281`; current local/hosted E2E Provider/harness lock `5aae281`/`9eb32ba` | Locks browser capability/create/session/handoff/usage/security authority and Go DTO/admission projection. Exact sandboxed signed amd64/arm64/v8 image publication and Browser runtime components are recorded separately. Browser protected handlers and caller-owned Gateway have distinct component evidence; no Browser command/runtime composition, public caller route, advertisement, Browser external caller, deployment, or production claim is implied |
...
| Browser-lock reference regression evidence | latest local and hosted harness/Provider `9eb32ba`/`5aae281`; local run `20260904T013909.243838000Z`; hosted run `33826813099` | Local and hosted Reference E2E passed all 15 initial and 5 process-reconstruction/resume coding/shell scenarios. The local runtime digest is `sha256:41c69ff79b9f895fa59e4a36d990993dffe0210b8b96df0bbf0647ae2ee651b4`; hosted artifact `reference-e2e-evidence-33826813099` has digest `sha256:dbe54035cd65ce60dcb1a45254d394dc04b0e530907363ea167f8502fd91fd8e`. This is reference coding/shell evidence, not Browser E2E |
| Browser-lock Agent Platform candidate regression evidence | latest local and hosted harness/Provider `9eb32ba`/`5aae281`; local run `20260904T014037.825223000Z`; hosted run `33826813100` | Local and hosted candidate E2E passed all 15 initial plus 5 resume coding/shell scenarios, including candidate shadow/selection/rollback/drain policy and reconstruction. The local runtime digest is `sha256:41c69ff79b9f895fa59e4a36d990993dffe0210b8b96df0bbf0647ae2ee651b4`; hosted artifact `platform-candidate-e2e-evidence-33826813100` has digest `sha256:f9a4f448e741c7945b225dd9dd1b72cf33d32745deb90a734b67fb41531f18bf`. This is candidate integration only, not Browser evidence, real Veronica, or production evidence |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, and admission bindings are locked. Provider-local Browser session, protected handler, and caller-owned Gateway components have separately bounded evidence; startup command/runtime composition, public caller edge, advertisement, and browser caller evidence remain open |
...
| P4 browser restricted-egress component | implementation `7e60340`; E2E lock refresh `7f15628`; ADR 0022; CI `33747803509`; local Gateway image `sha256:202bbf92fcbcce87e4b800f093d1df281125ab6fa43152906564cf8e0b7021d6` | A strict local policy registry, DNS/HTTP/TLS Gateway, per-allocation internal Docker bridge, explicitly owned uplink, immutable lifecycle/create policy binding, Browser-only DNS/network projection, policy/lease/ownership recovery, and exact cleanup pass focused/full race-shuffle, vet, unchanged Contract verification and 48-case Suite. The combined tagged integration uses the real `gh` verifier and signed Browser image, proves allowed HTTP/HTTPS, denied unlisted/metadata targets, process reconstruction, and no residual managed resources. Repository CI passes its four named jobs, but its Docker job does not execute this Browser tagged case. This remains single-controller local Docker component/lifecycle-binding evidence; the Gateway image has no publication/deployment provenance. Protected handlers and caller-owned Gateway are evidenced separately in `b8423f5` and `5aae281`; command/runtime composition, public caller edge, advertisement, Browser external caller, aggregate, multi-controller, tenant, deployment, and production gates remain open |
| P4 browser protected transport component | implementation `b8423f5`; ADR 0023; harness lock `a2721ad`; CI `33760609353`; hosted regressions `33760609272`/`33760609231` | The two Contract-authorized Browser routes now pass through mTLS/JWS/admission binding, bounded closed-JSON preflight, replay/fencing, application correlation, async operation projection, expiry, opaque handoff projection, and bounded error mapping. Nil application and malformed application identity fail closed; Browser operation-family aggregation is covered. Full local and hosted race/shuffle, vet, Contract verification, unchanged 48-case Suite, Docker regression, provenance, and locked 15+5 coding/shell caller regressions pass. This is Provider admission/transport component evidence only; the caller runs contain no Browser scenario, `cmd/serve` injects no Browser application, and command/runtime composition, advertisement, Browser external caller, aggregate, multi-controller, tenant, deployment, and production gates remain open |
| P4 caller-owned Browser Gateway component | implementation `5aae281`; ADR 0024; E2E lock `9eb32ba`; CI `33826813073`; hosted regressions `33826813099`/`33826813100` | The shared Gateway policy state machine now enforces exclusive terminal/Browser session identity and reference namespaces and exact resolved endpoint binding. The separate Browser composition requires caller authorization, revocation, recording, WebSocket admission, Provider resolution, and bounded reconnect settings; its private adapter validates RFC 6455 masking, fragmentation, control frames, UTF-8, cancellation, partial writes, and the 64 KiB hard message limit without auditing CDP payloads. Focused/full race-shuffle, vet, Contract verification, unchanged 48-case Suite, independent-module race/vet, and local/hosted 15+5 Reference/Candidate coding/shell regressions pass. This is an uncomposed component; no public route, Browser command/runtime graph, Browser caller, advertisement, aggregate, multi-controller, tenant, deployment, or production evidence is claimed |
...
| P3 candidate integration evidence | latest local and hosted harness/Provider `9eb32ba`/`5aae281`; local evidence `e2e/evidence/20260904T014037.825223000Z`; hosted run `33826813100` | Current local and hosted candidate lock regressions pass 15+5 coding/shell scenarios against Contract 48 cases. Hosted artifact digest is `sha256:f9a4f448e741c7945b225dd9dd1b72cf33d32745deb90a734b67fb41531f18bf`. Boundary is `Agent Platform candidate integration only`; Browser scenarios, real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | latest local and hosted harness/Provider `9eb32ba`/`5aae281`; local evidence `20260904T013909.243838000Z`; hosted run `33826813099` | Local and hosted runs passed 15 initial plus 5 restart/resume coding/shell scenarios. Hosted artifact digest is `sha256:dbe54035cd65ce60dcb1a45254d394dc04b0e530907363ea167f8502fd91fd8e`. They contain no Browser scenario and prove no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production property |
| P4 browser Contract authority | Contract `5096e71`; projection/lock `24b2e36`; current local E2E lock `9eb32ba` | Browser-only capability/create/session/handoff/usage/security/admission authority, DTO projection, fail-closed shapes, protected handler and caller-owned Gateway component evidence, and 10 new Suite cases pass locally. The image and uncomposed Provider-local session, Docker/private-relay, provenance, and restricted-egress components are recorded separately; startup application composition, public caller route, advertisement, and Browser E2E remain absent |
...
| P2.5i | Independent reference caller release gate | Latest local run `20260904T013909.243838000Z` and hosted run `33826813099` with clean co-located harness `9eb32ba` passed Provider lock `5aae281`, recording 15 initial and 5 restart/resume coding/shell passes | The local runtime digest is `sha256:41c69ff79b9f895fa59e4a36d990993dffe0210b8b96df0bbf0647ae2ee651b4`; hosted artifact digest is `sha256:dbe54035cd65ce60dcb1a45254d394dc04b0e530907363ea167f8502fd91fd8e`. Neither run contains a Browser scenario and neither may be promoted to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P3 | Migration readiness and platform integration | Local component evidence (`4212e88`) plus latest local candidate integration (`20260904T014037.825223000Z`, `9eb32ba`/`5aae281`, 15+5 coding/shell) and hosted candidate run `33826813100` against the same lock are present; the reference P2 caller gate passes | Supply a real platform migration target and prove locked-Suite/request shadow parity, new-run-only canary, rollback, old-run drain, metric parity, and unchanged platform-owned contracts |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, exact sandboxed signed linux/amd64 plus linux/arm64/v8 publication, Provider-local browser session/application/reference/usage components, fail-closed Docker/private-relay, provenance, restricted-egress/create-policy, protected transport, and caller-owned Gateway components have named evidence | Compose the complete Browser command/runtime graph and explicit public caller edge, then establish Browser caller evidence before advertisement |
...
- P4 Browser protected-transport verification covers the two locked route
  ...
  command/runtime graph and is not Browser
...
- Browser command/runtime composition, explicit public caller-edge integration,
...
`24b2e36`; latest local and hosted E2E evidence `9eb32ba`/`5aae281` proves only
15+5 coding/shell regression. ADR 0019
and publication
...
ADR 0024 and implementation `5aae281` then add the uncomposed caller-owned
Browser Gateway component; E2E lock `9eb32ba` passes full local gates and fresh
Reference/Candidate runs `20260904T013909.243838000Z` and
`20260904T014037.825223000Z`, each with 15+5 coding/shell scenarios. These runs
contain no Browser scenario. Repository CI `33826813073` and hosted
Reference/Candidate workflows `33826813099`/`33826813100` pass against the same
lock; their caller runs also contain no Browser scenario. Browser
command/runtime composition, public caller-edge integration, advertisement,
