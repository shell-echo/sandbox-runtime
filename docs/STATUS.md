...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy implementation `7e60340`; latest hosted E2E Provider/harness lock `9390554`/`330f629` | Locks browser capability/create/session/handoff/usage/security authority and Go DTO/admission projection. Exact sandboxed signed amd64/arm64/v8 image publication and the uncomposed Browser runtime components are recorded separately. Both browser routes remain uncomposed and return `404`; no handler, advertisement, Browser external caller, deployment, or production claim is implied |
...
| Browser-lock reference regression evidence | latest local harness/Provider `330f629`/`9390554`, run `20260903T091011.164430000Z`; hosted run `33737531617` | Local and hosted Reference E2E passed all 15 initial and 5 process-reconstruction/resume coding/shell scenarios. Artifact `reference-e2e-evidence-33737531617` digest is `sha256:1a7c63d60c3dc61e9acd3057b9218ca85269d61e53157222a93458c4372746fe`; its manifest pins Contract 48 cases. This is reference coding/shell evidence, not Browser E2E |
| Browser-lock Agent Platform candidate regression evidence | latest local harness/Provider `330f629`/`9390554`, run `20260903T091045.591005000Z`; hosted run `33737531705` | Local and hosted candidate E2E passed all 15 initial plus 5 resume coding/shell scenarios, including candidate shadow/selection/rollback/drain policy and reconstruction. Artifact `platform-candidate-e2e-evidence-33737531705` digest is `sha256:52ad3fcd3c5c1477be06bc270c9ace9e5988e737d7c4d475ce22ee7267af0ab0`. This is candidate integration only, not Browser evidence, real Veronica, or production evidence |
...
| P3 candidate integration evidence | harness `330f629`; Provider lock `9390554`; local evidence `e2e/evidence/platform-candidate/20260903T091045.591005000Z`; hosted run `33737531705` | Current local and hosted candidate lock regressions pass 15+5 coding/shell scenarios against Contract 48 cases. Artifact digest is `sha256:52ad3fcd3c5c1477be06bc270c9ace9e5988e737d7c4d475ce22ee7267af0ab0`. Boundary is `Agent Platform candidate integration only`; Browser scenarios, real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | co-located harness `330f62929d24614011c414b133198a03f267388d`; Provider lock `9390554`; local `20260903T091011.164430000Z`; hosted run `33737531617` | Local and hosted runs passed 15 initial plus 5 restart/resume coding/shell scenarios against the current lock. Artifact digest is `sha256:1a7c63d60c3dc61e9acd3057b9218ca85269d61e53157222a93458c4372746fe`. They contain no Browser scenario and prove no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production property |
| P4 browser Contract authority | Contract `5096e71`; projection/lock `24b2e36`; E2E lock `330f629` | Browser-only capability/create/session/handoff/usage/security/admission authority, DTO projection, fail-closed shapes, route absence, and 10 new Suite cases pass locally. The image and uncomposed Provider-local session, Docker/private-relay, provenance, and restricted-egress components are recorded separately; handler, advertisement, caller-owned Gateway, and Browser E2E remain absent |
...
| P2.5i | Independent reference caller release gate | Hosted run `33737531617` with clean co-located harness `330f629` passed Provider lock `9390554`, recording 15 initial and 5 restart/resume coding/shell passes | Artifact digest is `sha256:1a7c63d60c3dc61e9acd3057b9218ca85269d61e53157222a93458c4372746fe`. The run contains no Browser scenario and must not be promoted to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P3 | Migration readiness and platform integration | Local component evidence (`4212e88`) plus latest local candidate integration (`20260903T091045.591005000Z`, `330f629`/`9390554`, 15+5 coding/shell) and hosted candidate regression (run `33737531705`) are present; the reference P2 caller gate passes | Supply a real platform migration target and prove locked-Suite/request shadow parity, new-run-only canary, rollback, old-run drain, metric parity, and unchanged platform-owned contracts |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, exact sandboxed signed linux/amd64 plus linux/arm64/v8 publication, uncomposed Provider-local browser session/application/reference/usage components, a fail-closed Docker/private-relay adapter, real provenance verifier, restricted-egress provisioner, and create-policy binding have named evidence | Compose protected Browser handlers and the caller-owned Gateway, then establish Browser caller evidence before advertisement |
...
- P4 restricted-egress/create-policy verification covers strict policy/config
  ...
  single-controller local component/lifecycle-binding evidence only.
...
- Browser protected handler composition, caller-owned Gateway composition,
...
`24b2e36`; latest hosted and local E2E evidence `330f629`/`9390554` proves only
...
components, the fail-closed Docker/private-relay adapter, and the real
...
metadata denial, reconstruction, and cleanup. This remains local
single-controller component/lifecycle-binding evidence. Protected/Gateway
composition, handler routes, advertisement, Browser caller,
real platform, aggregate conformance, multi-controller, multi-tenant,
deployment, and production evidence remain independent open gates.
