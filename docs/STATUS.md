...
| ADR 0034 witnessed action-history component and local caller evidence | ADR 0034; witnessed v2 Redis adapter and Darwin/Linux file witness; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`; Contract `5096e71fb84fbec22aa3487a0e55a1b49602ab8b`; tree `859f76dc0e855a0c8abdbbb5648df100dabb4328`; harness/run `d61cd86`/`20260906T092259.737890000Z` | Local focused/full race-shuffle, vet, Contract lock verification, unchanged 48-case Suite, and tagged pinned-Valkey package integration pass. V2 retains at most 4,096 fingerprint-only session records in one permanent hash, rejects missing/malformed history, and compares its sequence/token with an independent monotonic witness; only an exact Redis-ahead-one interrupted commit may reconcile forward. The clean `linux/arm64` runner passes 18/18 through two Gateways, two independent callers, one unique ingress, real Chromium, a separate orchestrator fault credential, and a `0600` file witness outside its logical Valkey restore set. It proves the named deletion, restored-old-snapshot, witness-reconstruction, and checkpoint-mismatch behaviors before rejected actions open a new upstream dial. Exactly five `0600` evidence files pass cleanup and sanitization. This closes only the local ADR 0034 caller gate; the Suite remains unexercised. The file witness remains single-process component infrastructure, not production storage. Hosted v2, production witness/storage, Valkey provenance/HA/failover, production ACL separation, configuration/advertisement/deployment, aggregate, multi-controller, hostile multi-tenant, and production-readiness results remain absent |
| Current hosted CI and evidence tracks | Hosted checkout/harness `2cadc53`; CI `34013982778`; Reference `34013982794`; Candidate `34013982784`; Browser `34013982785`; shared capacity `34013982786`; durable revocation `34013982798`; downstream fencing `34013982796`; downstream artifact `browser-downstream-fencing-e2e-evidence-34013982796` | All seven workflows pass on the same checkout. The downstream run independently passes 13/13 on `linux/amd64`; its GitHub artifact digest is `sha256:9c00f3ba184e82d7eff661b831c05c1bdf331fa41caf2d8bbb3353168ab155b0`. CI and all six E2E profiles remain distinct evidence tracks. Their partial properties cannot be composed into aggregate conformance, real-platform compatibility, Provider multi-controller reliability, hostile multi-tenant security, deployment, or production readiness |
...
| P4 Browser witnessed action-history component and local caller evidence | ADR 0034; witnessed v2 implementation; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`; harness/run `d61cd86`/`20260906T092259.737890000Z` | The explicit v2 successor adds permanent bounded fingerprint-only history, strict whole-hash validation, an independent monotonic checkpoint witness, exact one-step interrupted-CAS recovery, and stable fail-closed errors. Focused/full race-shuffle, vet, Contract verification, the unchanged 48-case Suite, and tagged real-Valkey adapter integration pass. It does not rewrite v1 or alter default composition. The separately locked real-Chromium caller harness passes 18/18 locally on `linux/arm64`, with exactly five `0600` evidence files and all cleanup/sanitization checks true. This closes the local deletion/restore caller gate only; the Suite is not executed. The included file witness is local single-process infrastructure, not HA or production storage. Hosted v2, production witness, operational restore, deployment, and production gates remain open |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, signed image publication, Provider-local components, separately named Browser/shared-capacity/durable-revocation caller gates, ADR 0033 component/caller evidence, and ADR 0034 witnessed-v2 component evidence have passed within their tiers. The ADR 0033 runs remain v1; the separate v2 runner passes 18/18 locally on `linux/arm64`, and neither downstream profile executes the Contract Suite | Run and inspect the hosted v2 real-Chromium deletion/restore caller gate; production witness/storage and restore operations, Valkey provenance/HA, production configuration/metrics, hostile-tenant/operational evidence, deployment, and production advertisement remain later gates |
...
- ADR 0034 adds an explicitly selected v2 successor without changing those v1
  ...
  fault identity. Clean local run `20260906T092259.737890000Z` at harness
  `d61cd86` passes 18/18 on `linux/arm64`; its exact five-file `0600` evidence
  set passes cleanup and sanitization. This closes only the local ADR 0034
  Chromium deletion/restore caller gate. The Suite remains unexercised, and
  hosted v2, production witness, HA/failover, production ACL, deployment, and
...
- hosted execution and artifact inspection of the locked v2 multi-process
  real-Chromium caller gate; Browser production
  advertisement and deployable
...
Local ADR 0034 harness/run `d61cd86`/`20260906T092259.737890000Z` passes all 18
...
Next, run and inspect the hosted ADR 0034 v2 gate and its artifact.
