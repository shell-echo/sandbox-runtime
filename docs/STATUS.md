...
| ADR 0035 PostgreSQL witness candidate | ADR 0035; `PostgresActionHistoryWitness`; `pgx/v5 v5.9.2`; migration `gateway/capacity/redis/migrations/0001_action_history_witness.sql`; strict `VerifyRestoredState`; workflow `postgres-witness-integration.yml` | Namespace/policy digest isolation, idempotent provision, atomic conditional CAS, primary-local synchronous commit, bounded contexts, error redaction, schema constraints, and least-privilege role shape are implemented. Local unit/full race-shuffle, vet, Contract lock, unchanged 48-case Suite, tagged-integration compilation, and the pinned-Valkey strict one-ahead non-mutation scenario pass. The local environment had no PostgreSQL service and could not fetch its image, and the new hosted workflow has not run on this revision, so real PostgreSQL semantics, workflow success, image provenance, independent failure/backup domains, controlled ingress operations, HA/failover, deployment, and production readiness remain unproved |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, signed image publication, Provider-local components, separately named Browser/shared-capacity/durable-revocation caller gates, ADR 0033 component/caller evidence, and ADR 0034 witnessed-v2 component evidence have passed within their tiers. ADR 0035 adds a PostgreSQL witness candidate and strict restore primitive with local semantic evidence, but its real PostgreSQL workflow has not run. The ADR 0033 runs remain v1; the separate v2 runner passes 18/18 locally on `linux/arm64` and hosted on `linux/amd64`, and neither downstream profile executes the Contract Suite | Real PostgreSQL evidence, production independent witness/storage and restore operations, Valkey/PostgreSQL provenance/HA, production configuration/metrics, hostile-tenant/operational evidence, deployment, and production advertisement remain later gates |
