...
| --- | --- | --- |
| Latest verified implementation/evidence baseline | P2.5f1 implementation `6778d3c`; evidence baseline `66dd3d1` | Full race/shuffle, vet, module and Contract verification, locked 38-case Suite, focused terminal runtime/broker tests, both real Docker integration packages, and diff checks passed locally. Repository CI `33033284420` passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `66dd3d1`. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
