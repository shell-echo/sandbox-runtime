...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `9ff2b15` against Provider `d58497e`; evidence `20260831T033558.195791000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, Docker, and a temporary-registry named digest | Rerun the root `.github/workflows/reference-e2e.yml` after the temporary-registry fix and retain its artifact. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
