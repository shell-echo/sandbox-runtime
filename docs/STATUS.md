...
| Latest reference external-caller evidence | co-located `e2e/` harness `7ed591fc26da259857b14e082bf09a0dbcb6825d`; ignored evidence `20260831T030259.957424000Z` | Against Provider `d58497e`, all 15 initial and 5 process-reconstruction/resume scenarios passed over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. The manifest pins both commits, Contract revision/tree, 38-case Suite identity, configuration digests, and a local tag plus immutable digest. This is P2.5i reference caller evidence only; hosted rerun after the cross-Docker image-ID fix is pending |
...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `7ed591f` against Provider `d58497e`; evidence `20260831T030259.957424000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, and Docker | Rerun the root `.github/workflows/reference-e2e.yml` after the cross-Docker raw image-ID fix and retain its artifact. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
- successful execution and publication of the co-located, independently
  ...
  `.github/workflows/reference-e2e.yml`; hosted runs `33351621726` and
  `33352179181` failed because locally built Docker images had no registry
  RepoDigest, and fix `7ed591f` now uses the inspected raw image ID when that
  metadata is absent. The clean local 15+5 scenario result is external-caller
  evidence, while the Provider command root correctly remains free of
  caller-owned authorization, revocation, recording, and public Gateway
  defaults;
...
`.github/workflows/reference-e2e.yml`, but it has not yet produced a hosted CI
result.
...
| Input | Locked value |
| --- | --- |
| Caller harness | `7ed591fc26da259857b14e082bf09a0dbcb6825d` |
...
| Evidence directory | `20260831T030259.957424000Z` under `e2e/evidence/` (ignored) |
