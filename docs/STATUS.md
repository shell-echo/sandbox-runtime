...
| --- | --- | --- |
| Latest local implementation evidence | P2.5f6 implementation `c4c7cbc`; startup/locking regression `8a794c0` | Default-disabled development terminal configuration now composes durable session/reference files, the Docker terminal runtime, recovery before protected Provider session injection, idempotent opaque-reference completion, and bounded shutdown cleanup. Full race/shuffle, vet, unchanged Contract verification, the locked 38-case Suite, live Docker terminal regressions, and diff checks passed. This is single-controller local command-composition evidence only; it adds no public Gateway route, caller policy, capability advertisement, external caller, aggregate, multi-controller, multi-tenant, deployment, or readiness claim |
| Latest repository CI evidence | P2.5f5 implementation `fdfc771`; evidence baseline `754c57d` | Repository CI `33067526022` passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `754c57d`, which contains the P2.5f5 implementation. P2.5f6 CI is pending. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f6 | Development terminal command configuration and process lifecycle | Local evidence passes in `c4c7cbc` and `8a794c0`; repository CI pending | Retain default-disabled/single-controller/no-public-Gateway boundary; complete f7 terminal vertical evidence, artifact/usage composition, readiness-derived advertisement, then independent caller E2E |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1-f5 pass locally and in CI, and P2.5f6 has local command-composition evidence. No separately supplied caller is runnable | Complete P2.5f7-h Provider vertical evidence and CI, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root now composes
  ...
  profile. P2.5f0-f5 retain their earlier local/CI evidence; P2.5f6 local
  command-composition evidence passes and awaits CI. P2.5f7-h remain open;
