...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; latest hosted E2E Provider/harness lock `cd33ba3`/`e1de512` | Locks browser capability/create/session/handoff/usage/security authority and Go DTO/admission projection. Exact sandboxed signed amd64/arm64/v8 image publication and the uncomposed Browser Docker adapter are recorded separately. Both browser routes remain uncomposed and return `404`; no handler, advertisement, Browser external caller, deployment, or production claim is implied |
...
| Browser-lock reference regression evidence | latest local harness/Provider `e1de512`/`cd33ba3`, run `20260903T081108.412519000Z`; hosted run `33732133556` | Local and hosted Reference E2E passed all 15 initial and 5 process-reconstruction/resume coding/shell scenarios. Artifact `reference-e2e-evidence-33732133556` digest is `sha256:99f610aece673129923a36e176d11d90e10ca5587dec6a0c017b0db0a7a3ac34`; its manifest pins Contract 48 cases. This is reference coding/shell evidence, not Browser E2E |
| Browser-lock Agent Platform candidate regression evidence | latest local harness/Provider `e1de512`/`cd33ba3`, run `20260903T081149.245310000Z`; hosted run `33732133569` | Local and hosted candidate E2E passed all 15 initial plus 5 resume coding/shell scenarios, including candidate shadow/selection/rollback/drain policy and reconstruction. Artifact `platform-candidate-e2e-evidence-33732133569` digest is `sha256:41b42b9f9fe7060e9ac1a74a676fe28cf89416bcdabb231fcefe95d111c1bf5d`. This is candidate integration only, not Browser evidence, real Veronica, or production evidence |
...
| P3 candidate integration evidence | harness `e1de512`; Provider lock `cd33ba3`; local evidence `e2e/evidence/platform-candidate/20260903T081149.245310000Z`; hosted run `33732133569` | Current local and hosted candidate lock regressions pass 15+5 coding/shell scenarios against Contract 48 cases. Artifact digest is `sha256:41b42b9f9fe7060e9ac1a74a676fe28cf89416bcdabb231fcefe95d111c1bf5d`. Boundary is `Agent Platform candidate integration only`; Browser scenarios, real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | co-located harness `e1de51277c348810b1659b5643a281a622464c92`; Provider lock `cd33ba3`; local `20260903T081108.412519000Z`; hosted run `33732133556` | Local and hosted runs passed 15 initial plus 5 restart/resume coding/shell scenarios against the current lock. Artifact digest is `sha256:99f610aece673129923a36e176d11d90e10ca5587dec6a0c017b0db0a7a3ac34`. They contain no Browser scenario and prove no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production property |
| P4 browser Contract authority | Contract `5096e71`; projection/lock `24b2e36`; E2E lock `e1de512` | Browser-only capability/create/session/handoff/usage/security/admission authority, DTO projection, fail-closed shapes, route absence, and 10 new Suite cases pass locally. The image, uncomposed Provider-local session components, and Docker/private-relay adapter are recorded separately; handler, advertisement, Gateway, and Browser E2E remain absent |
...
| P2.5i | Independent reference caller release gate | Hosted run `33732133556` with clean co-located harness `e1de512` passed Provider lock `cd33ba3`, recording 15 initial and 5 restart/resume coding/shell passes | Artifact digest is `sha256:99f610aece673129923a36e176d11d90e10ca5587dec6a0c017b0db0a7a3ac34`. The run contains no Browser scenario and must not be promoted to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P3 | Migration readiness and platform integration | Local component evidence (`4212e88`) plus latest local candidate integration (`20260903T081149.245310000Z`, `e1de512`/`cd33ba3`, 15+5 coding/shell) and hosted candidate regression (run `33732133569`) are present; the reference P2 caller gate passes | Supply a real platform migration target and prove locked-Suite/request shadow parity, new-run-only canary, rollback, old-run drain, metric parity, and unchanged platform-owned contracts |
...
`24b2e36`; latest hosted and local E2E evidence `e1de512`/`cd33ba3` proves only
