...
- Current slice: P1.1c.4a durable single-controller mutation guard
- P1.1 release gate: open
- Slices in progress: P1.1c.4a adds a local durable replay/fencing guard with
  an exclusive single-controller lock, bounded versioned state, expiry pruning,
  atomic replacement, and no raw JTI/bearer/request/lifecycle retention. The
  caller value must be the unique allowlisted URI SAN selected by TLS
  verification, but no protected transport passes that fact yet. Protected
  routes and P1.2 lifecycle remain unimplemented.
...
application-level context/time binding, and a durable single-controller guard.
The next unit is protected transport composition, proving every rejection
occurs before any repository or driver dispatch. The detailed scope, acceptance
