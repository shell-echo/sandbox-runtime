...
| --- | --- | --- |
| Latest verified implementation/evidence baseline | P2.5f2 implementation `ccffd52`; evidence baseline `3abefd4` | Full race/shuffle, vet, module and unchanged Contract verification, locked 38-case Suite, focused session coordination/restart/migration tests, both real Docker integration packages, and diff checks passed locally at `ccffd52`. Repository CI `33045725476` passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `3abefd4`. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f2 | Durable session coordination and repository migration | Passed local and repository CI gates in implementation `ccffd52`, evidence baseline `3abefd4`, and CI `33045725476` | Implement P2.5f3 opaque reference registry/resolver. Retain the single-controller, separate-repository, no-command-composition boundary |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, and P2.5f1-f2 pass locally and in CI. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f3-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  pass; P2.5f3 opaque reference persistence/resolution is the next
