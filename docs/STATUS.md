...
| --- | --- | --- |
| Latest verified implementation/evidence baseline | evidence `2f7a79b`; P2.5e implementation `5917a57` | Full race/shuffle, vet, Contract verification, locked 38-case Suite, focused exec/restart/cancellation tests, both real Docker integration packages, and diff checks passed locally. Post-push CI `32937530059` passed `provider-contract`, `test`, and `docker-integration`. Verify the live checkout independently; this evidence baseline is intentionally not a self-updating `HEAD` claim |
...
| P2.5e | Exec vertical composition | Passed in `5917a57`; evidence baseline `2f7a79b` and CI `32937530059` passed | Retain exec admission/runtime/recovery matrices and empty advertisement. P2.5f0 design audit is complete and P2.5f1 reconnectable terminal runtime is next; external caller, aggregate, reliability, tenancy, deployment, and production claims remain open |
| P2.5f0 | Terminal/Gateway authority, data-plane, recovery, and deployment audit | Complete as design evidence; no behavior or advertisement changed | Begin P2.5f1 with a genuinely reconnectable terminal runtime/broker port and Docker adapter; do not reuse one-shot Docker exec attach as reconnect evidence |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, and P2.5f0 has design evidence only. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f1-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  advertises no capability or runtime profile. P2.5f0 is complete as design
  evidence; P2.5f1 reconnectable terminal runtime is the next implementation
  slice;
