...
| --- | --- | --- |
| Latest verified local implementation baseline | P2.5f1 implementation `6778d3c`; latest repository CI evidence remains P2.5e baseline `2f7a79b` | Full race/shuffle, vet, module and Contract verification, locked 38-case Suite, focused terminal runtime/broker tests, both real Docker integration packages, and diff checks passed locally at `6778d3c`. Repository CI `32937530059` passed `provider-contract`, `test`, and `docker-integration` for older baseline `2f7a79b`; P2.5f1 is not pushed and has no CI result. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f1 terminal runtime local evidence | implementation commit `6778d3c` | Backend-neutral allocate/observe/attach/cleanup ports, PTY-owning guest broker, adapter-private Docker identity, opaque `ref:terminal/*` receipt, lost-start recovery, capacity, expiry, identity conflict, process-group cleanup, sandbox removal, cancellation, and corrupt-state tests passed. Full race/shuffle, vet, module and unchanged Contract verification, the locked 38-case Suite, diff checks, and live Docker same-shell reattach after Provider driver reconstruction passed locally. No repository CI, session coordinator, `ref:session:*` resolver, WebSocket/Gateway composition, external caller, or readiness evidence is claimed |
...
| P2.5f1 | Reconnectable terminal runtime port and Docker adapter | Passed local component and live Docker gates in `6778d3c`; no repository CI evidence yet | Begin P2.5f2 durable session coordination, repository migration, and lifecycle-authority synchronization. Do not promote runtime-adapter evidence to a terminal/Gateway vertical claim |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, and P2.5f1 passes locally. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f2-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2.5f1 local verification covers the backend-neutral terminal allocation,
  ...
  the broker into writable `/workspace` is test deployment only. This does not
  prove durable session coordination, `ref:session:*` resolution, WebSocket or
  Gateway composition, repository CI, external caller compatibility, aggregate
  conformance, multi-controller reliability, multi-tenant security, deployment,
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  advertises no capability or runtime profile. P2.5f0 design and P2.5f1 local
  runtime-adapter evidence pass; P2.5f2 durable session coordination and
  repository migration is the next implementation slice;
