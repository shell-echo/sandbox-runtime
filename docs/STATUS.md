...
| P4 browser Contract authority and reference callers | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; downstream component `b4d41c9`; local downstream harness/run `550c785`/`20260906T050213.016063000Z`; hosted checkout/downstream run `2cadc53`/`34013982796` | Browser authority and the Browser reference, shared-capacity, durable-revocation, and downstream-fencing tracks remain separate. The downstream track passes its 13-scenario caller gate and authenticated unique-ingress topology on local arm64 and hosted amd64. The 48-case Suite remains unexercised in this profile. Deleted/restore-missing high-water detection, restored-snapshot consistency, Valkey provenance/HA/failover, production storage/configuration/metrics/deployment and topology, real platform, aggregate, multi-controller, hostile multi-tenant, and production evidence remain absent |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, signed image publication, Provider-local components, separately named Browser/shared-capacity/durable-revocation caller gates, and ADR 0033 component evidence have passed within their tiers. Local harness/run `550c785`/`20260906T050213.016063000Z` and hosted harness/run `2cadc53`/`34013982796` also pass the 13-scenario two-Gateway/unique-ingress/retained-Valkey/signed-real-Chromium gate on arm64 and amd64; its Contract Suite remains unexercised | Deleted/restore-missing high-water controls, restored-snapshot consistency, Valkey provenance/HA, production configuration/metrics, hostile-tenant/operational evidence, deployment, and production advertisement remain later gates |
...
- Local harness/run `550c785`/`20260906T050213.016063000Z` and hosted
  ...
  caller gates. Prematurely deleted or restore-missing high-water detection,
  restored-snapshot consistency, Valkey provenance/HA/failover, production
  metrics/configuration/deployment, Provider multi-controller, hostile
...
- detection and control for prematurely deleted or restore-missing high-water
  state;
...
Next, implement detection/control for prematurely deleted or restore-missing
high-water state and restored-snapshot consistency. Valkey provenance and
HA/failover, production storage/configuration and metrics, hostile-tenant and
operational evidence, production Browser advertisement and deployable
unique-ingress/public Gateway, real Agent Platform, aggregate,
