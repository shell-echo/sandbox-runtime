...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; E2E lock `75e5725` | Locks browser capability/create/session/handoff/usage/security authority and Go DTO/admission projection. The two browser routes remain uncomposed and return `404`; no browser runtime, image, handler, advertisement, external caller, deployment, or production claim is implied |
| Latest repository CI evidence | hosted workflow baseline `b72fc3b96b63987477c793979dc7cc9ef8952905` | Repository CI run `33591808969` passed `provider-contract`, `test`, and `docker-integration`. Earlier P2.5h and P2.5g implementation gates remain in runs `33159099578` and `33157119149`. These are repository CI results only, not external-caller or production evidence |
| Latest reference external-caller evidence | co-located harness `75e572599907a1dc15199f245a5e2f1719d6d967`; local run `20260902T065111.030014000Z` | Against Provider `24b2e36`, all 15 initial and 5 process-reconstruction/resume coding/shell scenarios passed over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. The manifest pins Contract 48 cases and image digest `sha256:93b6504a...812c`. Hosted run `33591808946` remains the latest published artifact, on the previous lock; its artifact digest is `sha256:ae1cdeb1...1526`. This is reference coding/shell evidence, not browser E2E |
| Latest Agent Platform candidate integration evidence | co-located candidate harness `75e572599907a1dc15199f245a5e2f1719d6d967`; local run `20260902T065200.812211000Z` | The candidate passed lock verification and all 15 initial plus 5 resume coding/shell scenarios against Provider `24b2e36`, including candidate shadow/selection/rollback/drain policy and reconstruction. Hosted run `33591808961` remains the latest published candidate artifact, on the previous lock; its artifact digest is `sha256:781073ee...9d77`. This is candidate integration only, not browser evidence, real Veronica, or production evidence |
...
| P3 candidate integration evidence | implementation `75e5725`; local evidence `e2e/evidence/20260902T065200.812211000Z`; hosted baseline `b72fc3b`, run `33591808961` | Current local candidate lock and 15+5 coding/shell scenarios pass against Provider `24b2e36`/Contract 48 cases. Hosted evidence remains on the previous lock. Boundary is `Agent Platform candidate integration only`; browser scenarios, real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | co-located harness `75e572599907a1dc15199f245a5e2f1719d6d967`; Provider `24b2e36485c334634e561009850d1905ec3115d5`; local run `20260902T065111.030014000Z`; hosted run `33591808946` | Current local and previous hosted runs passed 15 initial plus 5 restart/resume coding/shell scenarios. The local manifest pins Contract 48 cases and a named linux/amd64 runtime digest. It contains no browser scenario and proves no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production property |
| P4 browser Contract authority | Contract `5096e71`; projection/lock `24b2e36`; E2E lock `75e5725` | Browser-only capability/create/session/handoff/usage/security/admission authority, DTO projection, fail-closed shapes, route absence, and 10 new Suite cases pass locally. No browser runtime/image/application/handler/advertisement or browser E2E exists |
...
| P2.5i | Independent reference caller release gate | Current local lock-refresh passed with clean co-located harness `75e5725` against Provider `24b2e36`; evidence `20260902T065111.030014000Z` records 15 initial and 5 restart/resume coding/shell passes and exact lock/config/image identities | Hosted run `33591808946` remains the latest published reference artifact on the previous lock. Current and hosted results contain no browser scenario and must not be promoted to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P3 | Migration readiness and platform integration | Local component evidence (`4212e88`) plus current local candidate integration (`20260902T065200.812211000Z`, 15+5 coding/shell) and previous Hosted candidate integration (`e5d7324`/`b72fc3b`) are present; the reference P2 caller gate passes | Supply a real platform migration target and prove locked-Suite/request shadow parity, new-run-only canary, rollback, old-run drain, metric parity, and unchanged platform-owned contracts |
...
| P4 | Optional capability profiles | Browser Contract authority/projection passed; runtime implementation has not started | Build and verify the browser image, then uncomposed runtime/session/evidence components. Add handler/advertisement and browser caller evidence only after their named gates |
...
only regression against the advanced identity. The next slice is a real,
reproducible browser runtime image/provenance gate followed by uncomposed
provider-local browser session, durable operation/evidence, expiry, cleanup,
usage, and opaque-resolution components. Browser handler composition,
advertisement, external caller, real platform, multi-controller, multi-tenant,
deployment, and production evidence remain later independent gates.
