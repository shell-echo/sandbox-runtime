...
| Latest reference external-caller evidence | co-located `e2e/` harness `e329150df3d33a21ba30c1f616a94246b4ff8804`; ignored evidence `20260831T094209.963835000Z` | Against Provider `d58497e`, all 15 initial and 5 process-reconstruction/resume scenarios passed locally over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker, including Gateway revocation after the bounded drain fix. Hosted run `33377404561` passed the first 11 initial scenarios with the pre-fix harness but failed because its one-shot Gateway revocation read treated a queued PTY frame as an open connection. A hosted rerun against `e329150` is pending; this remains P2.5i reference caller evidence only |
...
| P2.5i reference external caller | co-located harness `e329150df3d33a21ba30c1f616a94246b4ff8804`; Provider `d58497e5359056858564b9ac663178958cf5a6d6` | Clean local run passed 15 initial plus 5 restart/resume scenarios, including two controller contexts, endpoint non-disclosure, negative authorization, expiry/revocation, retained evidence, and same-shell reconnect. Hosted run `33377404561` passed 11 initial scenarios with the pre-fix harness but failed in caller verification because a single Gateway `Read` did not drain a queued PTY frame before close. The bounded drain fix passes locally; hosted rerun is pending. No Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production claim |
