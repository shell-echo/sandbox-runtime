...
| P4 browser protected transport component | implementation `b8423f5`; ADR 0023; local validation on 2026-09-03 | The two Contract-authorized Browser routes now pass through mTLS/JWS/admission binding, bounded closed-JSON preflight, replay/fencing, application correlation, async operation projection, expiry, opaque handoff projection, and bounded error mapping. Nil application and malformed application identity fail closed; Browser operation-family aggregation is covered. Full race/shuffle, vet, Contract verification, and the unchanged 48-case Suite pass locally. This is Provider admission/transport component evidence only; `cmd/serve` injects no Browser application, and caller-owned Gateway, advertisement, Browser external caller, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
- P4 Browser protected-transport verification covers the two locked route
  ...
  verification, and the unchanged 48-case Suite locally. This does not compose
  the Browser command/runtime graph or caller-owned Gateway and is not Browser
...
48-case Suite pass. This is Provider admission/transport component evidence.
