...
| --- | --- | --- |
| Latest downstream-fencing implementation and local caller evidence | Contract `5096e71`; projection/lock `24b2e36`; fencing component `b4d41c9`; Provider bootstrap `58488d7`; provisioning/process component `8a1049b`; current lock descriptor `b1d41f3`; harness `550c785`; local run `20260906T050213.016063000Z` | The component and real-backend integration gates remain valid. The clean `linux/arm64` run passes all 13 scenarios through two independent mTLS/JWS caller processes, two Gateway processes, one authenticated unique ingress, retained Valkey state, and signed real Chromium. It exercises six protected Provider routes and emits exactly five `0600` evidence files with all cleanup/sanitization checks true. This closes only the local ADR 0033 external-caller gate. Contract/tree/48-case identity is pinned but the Suite is not executed (`suite_exercised=false`). Hosted, prematurely deleted or restore-missing high-water detection, restored-snapshot consistency, Valkey provenance/HA/failover, production advertisement/public Gateway, real Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production readiness remain open |
| Preceding hosted regressions and downstream hosted status | Latest hosted regression checkout `c3e34ef` / E2E lock `f7de91d` / Provider `58488d7`; CI `33990418428`; Reference `33990418435`; Candidate `33990418458`; Browser `33990418425`; shared capacity `33990418420`; durable revocation `33990418412`; downstream local harness `550c785` | The six `c3e34ef` runs pass their existing CI and 15+5 Reference, 15+5 Candidate, 13+5 Browser, 10/10 shared-capacity, and 7/7 durable-revocation profiles only. They contain no ADR 0033 action-fence scenario. The downstream workflow exists, but no hosted `linux/amd64` result is yet claimed for `550c785`; local arm64 evidence cannot be promoted to hosted, real-platform, aggregate, multi-controller, multi-tenant, deployment, or production evidence |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; authenticated capacity `997fb0d`; Redis-compatible capacity `9434540`; ADR 0032 revocation component `c0a55d1`; durable-revocation harness/Gateway `e952ef9`; ADR 0033 component `b4d41c9`; downstream harness/run `550c785`/`20260906T050213.016063000Z` | Browser authority and all named component/caller gates remain separate. Local ADR 0033 passes 13/13 on `linux/arm64` for the locked two-Gateway/two-caller/unique-ingress/retained-Valkey/signed-real-Chromium topology. Its Contract identity is pinned, while the Suite remains unexercised. Hosted repetition, deleted/restore-missing high-water controls, restored-snapshot consistency, Valkey provenance/HA/failover, production storage/configuration/metrics/deployment, aggregate, multi-controller, hostile multi-tenant, real Agent Platform, and production gates remain open |
...
| P4 Browser downstream CDP fencing component and local external-caller evidence | ADR 0033; fencing implementation `b4d41c9`; Provider bootstrap `58488d7`; provisioning/process implementation `8a1049b`; lock descriptor `b1d41f3`; harness `550c785`; local `linux/arm64` run `20260906T050213.016063000Z`; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd` | The action-fence/private-ingress/Redis adapter passes its component plus real-backend integration gate. The clean E2E run passes 13/13 through two independent mTLS/JWS callers, two Gateways, one authenticated unique ingress, retained Valkey, and signed real Chromium. It proves ordinary/replacement mutations, stale and pre-action lease-loss rejection before Chromium, terminal lower-fence closure without reconnect, unaffected scopes, outage recovery, retained high-water across ingress reconstruction, bypass exclusion, and cleanup/sanitization. Exactly five `0600` files are retained; manifest cleanup and sanitization booleans are all true. Six protected Provider routes are exercised, but the 48-case Suite is not (`suite_exercised=false`). This closes only the local ADR 0033 caller gate. No hosted run, deleted/restore-missing high-water detection, restored-snapshot consistency, Valkey provenance/HA/failover, production metrics/configuration/deployment, Provider multi-controller reliability, hostile multi-tenant isolation, real Agent Platform compatibility, aggregate conformance, or production readiness is proved |
...
| P4 browser Contract authority and reference callers | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; downstream component `b4d41c9`; local downstream harness/run `550c785`/`20260906T050213.016063000Z`; latest verified hosted Browser/shared/durable runs `33990418425`/`33990418420`/`33990418412` at checkout/lock `c3e34ef`/`f7de91d` | Browser authority and the Browser reference, shared-capacity, durable-revocation, and downstream-fencing tracks remain separate. The local downstream track passes its 13-scenario arm64 caller gate and authenticated unique-ingress topology. No hosted downstream result is claimed; the 48-case Suite remains unexercised in this profile. Deleted/restore-missing high-water detection, restored-snapshot consistency, Valkey provenance/HA/failover, production storage/configuration/metrics/deployment and topology, real platform, aggregate, multi-controller, hostile multi-tenant, and production evidence remain absent |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, signed image publication, Provider-local components, separately named Browser/shared-capacity/durable-revocation caller gates, and ADR 0033 component evidence have passed within their tiers. Local harness `550c785` run `20260906T050213.016063000Z` also passes the 13-scenario two-Gateway/unique-ingress/retained-Valkey/signed-real-Chromium gate on `linux/arm64`; its Contract Suite remains unexercised. Latest preceding hosted regression evidence is checkout `c3e34ef` with lock `f7de91d` | Reproduce ADR 0033 on hosted `linux/amd64`. Deleted/restore-missing high-water controls, restored-snapshot consistency, Valkey provenance/HA, production configuration/metrics, hostile-tenant/operational evidence, deployment, and production advertisement remain later gates |
...
- Harness `550c785` local run `20260906T050213.016063000Z` passes all 13 ADR
  0033 scenarios on `linux/arm64` through two independent callers, two
  Gateways, one authenticated unique ingress, retained Valkey, and signed real
  Chromium. The five retained evidence files are `0600`; all cleanup and
  sanitization checks pass. Contract/tree/48-case identity is pinned, six
  protected Provider routes are exercised, and the Suite is explicitly not
  executed (`suite_exercised=false`). This closes only the local named caller
  gate. Hosted repetition, prematurely deleted or restore-missing high-water
  detection, restored-snapshot consistency, Valkey provenance/HA/failover,
...
- hosted `linux/amd64` repetition of the ADR 0033 caller gate; detection and
  control for prematurely deleted or restore-missing high-water state;
  restored-snapshot consistency; Browser production advertisement and
  deployable authenticated unique-ingress/public caller-owned Gateway; Valkey
  provenance and HA/failover consistency; production storage/configuration and
  metrics; and hostile-tenant and operational evidence;
...
all cleanup and sanitization checks. No hosted result is claimed yet.

The latest verified hosted regression checkout is `c3e34ef`, using E2E
lock `f7de91d` and Provider `58488d7`. Repository CI `33990418428` and Reference,
Candidate, Browser, shared-capacity, and durable-revocation runs `33990418435`,
`33990418458`, `33990418425`, `33990418420`, and `33990418412` all pass their
existing scenario sets. They contain no downstream action-fence scenario and
do not close the hosted ADR 0033 gate.

Next, reproduce the two-Gateway/unique-ingress/real-Valkey/signed-real-Chromium
13-scenario runner on hosted `linux/amd64` and inspect its sanitized artifact.
...
storage/configuration and metrics, hostile-tenant and operational evidence,
production Browser advertisement and deployable unique-ingress/public Gateway,
real Agent Platform, aggregate, multi-controller, multi-tenant, deployment, and
