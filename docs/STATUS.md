...
| Browser-lock reference regression evidence | hosted harness/Provider `6b01b75`/`997fb0d`, run `33940332897`; current local harness/Provider `59e08d5`/`c0a55d1`, run `20260905T080530.577843000Z` | Hosted and current local Reference E2E each passed all 15 initial and 5 process-reconstruction/resume coding/shell scenarios. Hosted artifact `reference-e2e-evidence-33940332897` has digest `sha256:c5294520236200c06597bcc383455b9a74cb3668557cce313ab7e4dab0af0537`; the local run pins runtime digest `sha256:c1da999518ed8cf3c422142b705f3eeca5d60104be07bc28c15a7d527d2907fc`. This is coding/shell regression evidence, not Browser E2E or durable-revocation caller evidence |
| Browser-lock Agent Platform candidate regression evidence | hosted harness/Provider `6b01b75`/`997fb0d`, run `33940332881`; current local harness/Provider `59e08d5`/`c0a55d1`, run `20260905T080623.861033000Z` | Hosted and current local candidate E2E each passed all 15 initial plus 5 resume coding/shell scenarios, including candidate shadow/selection/rollback/drain policy and reconstruction. Hosted artifact `platform-candidate-e2e-evidence-33940332881` has digest `sha256:5fad63cd7dfc863af83faf2e5324529852cc8b741deeac5414f12d5fd0dd26f5`; the local run pins runtime digest `sha256:c1da999518ed8cf3c422142b705f3eeca5d60104be07bc28c15a7d527d2907fc`. This is candidate integration only, not Browser evidence, real Veronica, durable-revocation caller evidence, or production evidence |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; authenticated capacity `997fb0d`; Browser caller harness `6b01b75`; Redis-compatible capacity `9434540`; shared Provider `2ed5e68`; local harness/run `ddbb2c4`/`20260905T061037.558537000Z`; hosted harness/run `de297e7`/`33949577876`; ADR 0032 revocation component `c0a55d1`; current local lock `59e08d5` | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, admission, default-disabled command composition, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 Browser caller evidence with the authenticated memory authority, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity evidence, and exact-grant revocation port/Redis-compatible adapter component evidence have named boundaries. Current-lock Browser 13+5, Reference/Candidate 15+5, and shared-capacity 10/10 local regressions pass but do not exercise distributed revocation. Production advertisement/public Gateway, the independent two-Gateway/independent-revoker durable-revocation caller gate, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, remaining profile-specific security/concurrency, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
| P2.5i | Independent reference caller release gate | Hosted run `33940332897` with harness/Provider `6b01b75`/`997fb0d` passed 15 initial and 5 restart/resume coding/shell scenarios; current local run `20260905T080530.577843000Z` passes the same set on harness/Provider `59e08d5`/`c0a55d1` | Hosted artifact digest is `sha256:c5294520236200c06597bcc383455b9a74cb3668557cce313ab7e4dab0af0537`; the local run pins runtime digest `sha256:c1da999518ed8cf3c422142b705f3eeca5d60104be07bc28c15a7d527d2907fc`. Neither run contains a Browser scenario and neither may be promoted to Agent Platform, durable-revocation caller, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
- P4 Browser exact-grant revocation verification covers the atomic
  ...
  revocation, expiry, capacity, and unavailability, and bounded audit reasons. ADR 0032 and
  implementation `c0a55d1` add the port, memory adapter changes, and the
  Redis-compatible retained-tombstone source/writer. The Redis adapter verifies
  immutable policy, uses hashed namespace/grant keys and Redis server time,
  enforces a 1-second to 24-hour maximum grant-lifetime policy range, preserves
  the maximum expiry with `PEXPIREAT`, and bounds polling and operations.
...
regressions pass. Those existing modes do not exercise two Gateways with an
independent revoker against the retained backend. Next work is therefore the
separately locked durable-revocation caller gate defined by ADR 0032, followed
