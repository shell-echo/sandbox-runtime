...
| --- | --- | --- |
| Last remote/CI implementation baseline | `b4c7e2d`; at audit time `main == origin/main` | `b4c7e2d` records the closed P2.4a3 CI evidence. P2.4a3 transport composition and the strict artifact-stage document preflight/error-category correction in `c469337` are merged. Local full gates and post-push CI `32838784395` (`provider-contract`, `test`, `docker-integration`) passed. Verify the live checkout independently; this evidence baseline is intentionally not a self-updating `HEAD` claim |
...
| P2.5b local Contract/projection evidence | Contract commit `22a148e`; projection/lock commit `123d16a` | Locks one atomic `sandbox.exec@1.0.0` + `sandbox.terminal@1.0.0` coding/shell profile and retains terminal-only compatibility. Empty, terminal-only, and complete coding/shell advertisements are accepted; exec-only, missing/split mapping, and unknown-capability shapes fail closed. Full race/shuffle, vet, lock verification, 38-case Suite, and diff checks passed locally. CI is pending, and no runtime behavior or nonempty startup advertisement was enabled |
...
| P2.5b | Coding/shell Contract profile reconciliation | Contract authority committed as `22a148e`; local projection/lock committed as `123d16a`; full local gates passed | Obtain CI evidence. Runtime composition and startup advertisement remain disabled; P2.5c mutation-preflight ordering is next |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a design evidence is accepted, and P2.5b local Contract/projection gates pass. The current command composition is not a real coding/shell Provider, and no separately supplied caller is runnable | Obtain P2.5b CI, complete P2.5c-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2.4a3 local verification covers the three locked artifact/usage routes,
  ...
  `32838784395` passed. Create/session schema-preflight ordering, external
  caller compatibility, aggregate conformance, multi-controller reliability,
  multi-tenant security, deployment, and production readiness remain unproven.
...
- P2.5b local verification covers Contract revision `22a148e2898477790512d5bb742605654ff00ebf`,
  ...
  Suite, and diff checks passed. CI, runtime dispatch, nonempty command
  advertisement, and external-caller evidence remain pending.
