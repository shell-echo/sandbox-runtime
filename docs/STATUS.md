...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy `7e60340`; protected transport `b8423f5`; caller-owned Gateway `5aae281`; default-disabled command graph `66183b1`; DNS compatibility Provider `f760369`; Browser Gateway local capacity `7b062e6`; Browser pre-upgrade edge `44ea2ee`; Browser public TLS edge `b8f8941`; authenticated capacity `997fb0d`; Redis-compatible shared capacity `9434540`; exact-grant revocation component `c0a55d1`; E2E lock `59e08d5`; hosted evidence baseline `c7fe24d` | Locks Browser authority, composes the default-disabled Provider-local graph, bounds process-local Gateway/pre-upgrade/listener work, adds caller-owned authenticated-capacity and exact-grant revocation ports, and implements Redis-compatible shared-capacity and retained-revocation adapters. Local component, Contract, unchanged 48-case Suite, real pinned-Valkey component integration, and current-lock local regressions pass. Hosted repository CI `33955437033` passes at repository revision `c7fe24d`. Separately named Reference/Candidate/Browser/shared-capacity runs `33955436969`/`33955437046`/`33955436984`/`33955436968` pass with harness `c7fe24d` locked to Provider `c0a55d1`. Production advertisement, Provider-owned public Gateway, the independent two-Gateway/independent-revoker durable-revocation caller gate, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, real Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production claims remain open |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; authenticated capacity `997fb0d`; Redis-compatible capacity `9434540`; ADR 0032 revocation component `c0a55d1`; current local lock `59e08d5`; hosted harness `c7fe24d` | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, admission, default-disabled command composition, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 Browser caller evidence, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity evidence, and exact-grant revocation port/Redis-compatible adapter component evidence have named boundaries. Current-lock local and hosted Browser 13+5, Reference/Candidate 15+5, and shared-capacity 10/10 regressions pass but do not exercise distributed revocation. Production advertisement/public Gateway, the independent two-Gateway/independent-revoker durable-revocation caller gate, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, remaining profile-specific security/concurrency, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
| P4 browser Contract authority and reference caller | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; capacity `7b062e6`; service edge `44ea2ee`; TLS edge `b8f8941`; authenticated capacity `997fb0d`; hosted Browser run `33955436984`; shared adapter `9434540`; local shared run `20260905T080725.227680000Z`; hosted shared run `33955436968`; revocation component `c0a55d1`; current local lock `59e08d5`; hosted harness `c7fe24d` | Browser authority, DTO projection, image/runtime components, default-disabled command graph, process-local Gateway limits, hosted 13+5 Browser scenarios with authenticated memory capacity, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity scenarios, and exact-grant revocation port/Redis-compatible adapter component tests pass within their names. The Browser, shared-capacity, revocation-component, and current-lock regression evidence tracks remain separate. The independent two-Gateway/independent-revoker durable-revocation caller gate, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, production advertisement/public Gateway, real platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production evidence remain absent |
...
- Contract/Suite component evidence alone establishes no external-caller E2E or
  ...
  Browser, and shared-capacity caller evidence is recorded below without
  establishing multi-controller reliability, multi-tenant safety, deployment
...
- P4 Browser exact-grant revocation verification covers the atomic
  ...
  authority, while shared capacity does not exercise revocation. No independent
  two-Gateway/independent-revoker durable-revocation caller gate, downstream
  fencing, Valkey provenance/HA, multi-controller, multi-tenant, deployment, or
...
- Browser production advertisement and deployable public caller-owned Gateway;
  the independent two-Gateway/independent-revoker durable-revocation caller
  gate; downstream fencing; Valkey provenance and HA/failover consistency;
  production storage/configuration and metrics; and hostile-tenant and
...
against the retained backend. Next work is therefore the separately locked
durable-revocation caller gate defined by ADR 0032, followed
separately by downstream CDP fencing, Valkey provenance and HA/failover
consistency, production storage/configuration and metrics, hostile-tenant and
operational evidence, and production Browser advertisement/public Gateway
review. Real Agent Platform, aggregate, multi-controller, multi-tenant,
deployment, and production gates remain separate.
