...
| Latest reference external-caller evidence | co-located `e2e/` harness `9ff2b150f7353399d3660334a057e105543428cc`; ignored evidence `20260831T033558.195791000Z` | Against Provider `d58497e`, all 15 initial and 5 process-reconstruction/resume scenarios passed locally over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. Hosted run `33377404561` passed the first 11 initial scenarios after the temporary-registry fix, then failed because the caller's one-shot Gateway revocation read treated a queued PTY frame as an open connection. The bounded drain fix is pending hosted rerun; this remains P2.5i reference caller evidence only |
...
| P2.5i reference external caller | co-located harness `9ff2b150f7353399d3660334a057e105543428cc`; Provider `d58497e5359056858564b9ac663178958cf5a6d6` | Clean local run passed 15 initial plus 5 restart/resume scenarios, including two controller contexts, endpoint non-disclosure, negative authorization, expiry/revocation, retained evidence, and same-shell reconnect. Hosted run `33377404561` passed 11 initial scenarios but failed in caller verification because a single Gateway `Read` did not drain a queued PTY frame before close. The harness now waits within a bounded window and needs a hosted rerun. No Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production claim |
...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `9ff2b15` against Provider `d58497e`; evidence `20260831T033558.195791000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, Docker, and a temporary-registry named digest | Hosted run `33377404561` reached 11 initial scenarios before the caller-side one-shot Gateway revocation read misclassified a queued PTY frame. Rerun after the bounded drain fix and retain its artifact. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
- successful execution and publication of the co-located, independently
  versioned P2.5i reference harness through
  `.github/workflows/reference-e2e.yml`; hosted runs `33351621726`,
  `33352179181`, and `33352689522` failed because locally built Docker images
  had no usable named `RepoDigest`. The temporary-registry fix enabled run
  `33377404561`, which passed 11 initial scenarios before the caller's one-shot
  Gateway revocation read misclassified a queued PTY frame as an open
  connection. The bounded drain correction and hosted rerun are pending. The
  clean local 15+5 scenario result is external-caller evidence, while the
...
`.github/workflows/reference-e2e.yml`; hosted run `33377404561` reached the
caller revocation scenario but failed on a caller-side one-shot read. The
bounded drain correction is pending rerun; hosted publication remains pending.

The final clean local run used:

| Input | Locked value |
| --- | --- |
| Caller harness | `9ff2b150f7353399d3660334a057e105543428cc` |
...
| Evidence directory | `20260831T033558.195791000Z` under `e2e/evidence/` (ignored) |
...
This is sufficient for the local reference P2.5i caller gate. Hosted run
`33377404561` did not complete because of the caller-side revocation
observation bug described above; rerun is required before hosted publication.
It is not evidence that the existing Agent Platform candidate is compatible,
