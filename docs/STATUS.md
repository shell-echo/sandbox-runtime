...
| Current E2E lock and hosted regressions | E2E lock/harness `17ed6ca`; Provider `b4d41c9`; CI `33970773423`; Reference `33970773414`; Candidate `33970773345`; Browser `33970773330`; shared capacity `33970773388`; durable revocation `33970773353` | CI passes `provider-contract`, `test`, `browser-provenance`, and `docker-integration`. The five `linux/amd64` workflows pass their existing 15+5 Reference, 15+5 Candidate, 13+5 Browser, 10/10 shared-capacity, and 7/7 durable-revocation sets. Artifact digests are `sha256:de2c6f2d31d9dd5a8323d82560bf0f314970c8e1825d815d224d48aac2dcba16`, `sha256:546c3ecd6f2cd23a629ab53eb61bbe23fa7361c8440297c0fb039886802d7bc0`, `sha256:38bb2d7edd0cf04350a3eb7974526296daf4b0cd58da7637ad889e4490c8d866`, `sha256:2350e3fbed3801366ade3bb4ed204545c1b0f55037a2dc7148de2ef4952e4216`, and `sha256:062bfea835758424eb399f22f63b78656b323c379d26e5cbaa78a9343fcc4eb9`, respectively. These are post-lock-refresh regressions only. No runner adds ADR 0033's two-Gateway/unique-ingress/real-Chromium action-fence scenario; shared/durable Contract identity remains `exercised=false`, and no real-platform, aggregate, multi-controller, multi-tenant, deployment, or production claim is added |
...
| P4 browser Contract authority and reference caller | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; capacity `7b062e6`; service edge `44ea2ee`; TLS edge `b8f8941`; authenticated capacity `997fb0d`; current E2E lock `17ed6ca`; hosted Browser run `33970773330`; shared adapter `9434540`; local shared run `20260905T080725.227680000Z`; hosted shared run `33970773388`; revocation component `c0a55d1`; hosted durable run `33970773353`; downstream fencing component `b4d41c9` | Browser authority, DTO projection, image/runtime components, default-disabled command graph, process-local Gateway limits, hosted 13+5 Browser scenarios with authenticated memory capacity, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity scenarios, exact-grant revocation component tests, local plus hosted seven-case two-Gateway/independent-revoker caller evidence, and downstream fencing component/real-backend adapter integration pass within their names. These evidence tracks remain separate. The current hosted workflows only rerun existing profiles. The ADR 0033 two-Gateway/unique-ingress/real-Chromium external-caller gate, private authenticated ingress topology, missing-high-water/restore controls, Valkey provenance/HA/failover, production storage/configuration/metrics/deployment, real platform, aggregate, multi-controller, hostile multi-tenant, and production evidence remain absent |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, exact sandboxed signed linux/amd64 plus linux/arm64/v8 publication, Provider-local components, default-disabled command graph, process-local Gateway limits, hosted 13+5 Browser caller run `33970773330`, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity evidence through run `33970773388`, ADR 0032 exact-grant revocation component and caller evidence through hosted run `33970773353`, and ADR 0033/`b4d41c9` downstream fencing component plus real-backend adapter integration have named boundaries. Lock `17ed6ca` adds regression evidence only | Run the independent ADR 0033 two-Gateway/unique-ingress/real-Chromium external-caller gate, then close private-ingress topology, high-water/restore, Valkey provenance/HA, production configuration/metrics, hostile-tenant/operational, and deployment gates before any production advertisement or Desktop authority audit |
...
E2E lock/harness `17ed6ca` subsequently pins Provider `b4d41c9`; repository CI
`33970773423` and hosted Reference, Candidate, Browser, shared-capacity, and
durable-revocation runs `33970773414`, `33970773345`, `33970773330`,
`33970773388`, and `33970773353` all pass their existing scenario sets. The
downloaded manifests and reports contain no downstream action-fence scenario.
This is regression evidence against the new implementation identity, not the
ADR 0033 caller gate.
