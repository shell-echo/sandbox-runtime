...
| Latest repository CI evidence | P2.5f5 implementation `fdfc771`; evidence baseline `754c57d` | Repository CI `33067526022` passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `754c57d`, which contains the P2.5f5 implementation. P2.5f6-f7 CI is not yet verified. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f6 | Development terminal command configuration and process lifecycle | Local evidence passes in `c4c7cbc` and `8a794c0`; repository CI unverified | Retain default-disabled/single-controller/no-public-Gateway boundary; f7 closes only the local terminal evidence gate, then work continues with artifact/usage, readiness-derived advertisement, and independent caller E2E |
| P2.5f7 | Terminal/Gateway vertical evidence gate | Local single-controller Docker/race/restart/reconnect gate passes in `0e8b284`; repository CI unverified | Retain same-repository test boundary and no-public-Gateway/no-external-caller claims; continue to P2.5g artifact/usage vertical |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1-f5 pass locally and in CI, and P2.5f6-f7 pass their local terminal composition/evidence gates. No separately supplied caller is runnable | Complete P2.5g-h Provider vertical evidence and CI, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root now composes
  ...
  profile. P2.5f0-f5 retain their earlier local/CI evidence; P2.5f6-f7 local
  command-composition and vertical evidence pass while repository CI remains
  unverified. P2.5g-h remain open;
