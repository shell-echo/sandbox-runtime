...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy `7e60340`; protected transport `b8423f5`; caller-owned Gateway `5aae281`; default-disabled command graph `66183b1`; DNS compatibility Provider `f760369`; Browser Gateway local capacity `7b062e6`; Browser pre-upgrade edge `44ea2ee`; Browser public TLS edge `b8f8941`; authenticated capacity `997fb0d`; Redis-compatible shared capacity `9434540`; exact-grant revocation component `c0a55d1`; earlier E2E lock `59e08d5`; durable-revocation harness/Gateway `e952ef9`; repository CI `33959122462`; hosted durable run `33959122456` | Locks Browser authority, composes the default-disabled Provider-local graph, bounds process-local Gateway/pre-upgrade/listener work, adds caller-owned authenticated-capacity and exact-grant revocation ports, and implements Redis-compatible shared-capacity and retained-revocation adapters. Local component, Contract, unchanged 48-case Suite, real pinned-Valkey component integration, and current-lock local regressions pass. Repository CI `33959122462` passes `provider-contract`, `test`, `browser-provenance`, and `docker-integration`. Separately named Reference/Candidate/Browser/shared-capacity runs retain their recorded boundaries. Clean local arm64 run `20260905T095109.569973000Z` and hosted amd64 run `33959122456` pass the separate seven-case two-Gateway/independent-revoker durable-revocation caller gate. Production advertisement, Provider-owned public Gateway, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, real Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production claims remain open |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; authenticated capacity `997fb0d`; Redis-compatible capacity `9434540`; ADR 0032 revocation component `c0a55d1`; durable-revocation harness/Gateway `e952ef9`; hosted run `33959122456` | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, admission, default-disabled command composition, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 Browser caller evidence, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity evidence, exact-grant revocation port/Redis-compatible adapter component evidence, and local plus hosted 7/7 durable-revocation caller evidence have named boundaries. Contract/tree/48 cases are metadata with `exercised=false` in the durable runner. Production advertisement/public Gateway, downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, remaining profile-specific security/concurrency, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
| P4 browser Contract authority and reference caller | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; capacity `7b062e6`; service edge `44ea2ee`; TLS edge `b8f8941`; authenticated capacity `997fb0d`; hosted Browser run `33955436984`; shared adapter `9434540`; local shared run `20260905T080725.227680000Z`; hosted shared run `33955436968`; revocation component `c0a55d1`; durable harness `e952ef9`; hosted durable run `33959122456` | Browser authority, DTO projection, image/runtime components, default-disabled command graph, process-local Gateway limits, hosted 13+5 Browser scenarios with authenticated memory capacity, local arm64 plus hosted amd64 10/10 real-Valkey two-Gateway shared-capacity scenarios, exact-grant revocation port/Redis-compatible adapter component tests, and local plus hosted seven-case two-Gateway/independent-revoker caller evidence pass within their names. The Browser, shared-capacity, revocation-component, and durable-revocation evidence tracks remain separate. Downstream fencing, Valkey provenance, HA/failover consistency, production storage/configuration and metrics, production advertisement/public Gateway, real platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production evidence remain absent |
...
- Browser production advertisement and deployable public caller-owned Gateway;
  downstream fencing; Valkey provenance and HA/failover consistency; production
  storage/configuration and metrics; and hostile-tenant and operational
  evidence;
...
`sha256:1384a4504725c90717a3a8da058713fb1b8ed763f2c941b961811eb8370b8600`.
This closes only ADR 0032's durable exact-grant caller gate. Next work is the
separate downstream CDP fencing ADR/gate, followed by Valkey provenance and
