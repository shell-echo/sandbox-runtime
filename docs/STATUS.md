...
| ADR 0034 witnessed action-history component and unexecuted caller harness | ADR 0034; witnessed v2 Redis adapter and Darwin/Linux file witness; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`; Contract `5096e71fb84fbec22aa3487a0e55a1b49602ab8b`; tree `859f76dc0e855a0c8abdbbb5648df100dabb4328`; separately locked 18-scenario v2 runner | Local focused/full race-shuffle, vet, Contract lock verification, unchanged 48-case Suite, and tagged pinned-Valkey package integration pass. V2 retains at most 4,096 fingerprint-only session records in one permanent hash, rejects missing/malformed history, and compares its sequence/token with an independent monotonic witness; only an exact Redis-ahead-one interrupted commit may reconcile forward. The new runner composes two Gateways, two independent callers, one unique ingress, real Chromium, a separate orchestrator fault credential, and a `0600` file witness outside its logical Valkey restore set. It has not run from a clean committed checkout, so it adds no v2 caller/Chromium deletion or restore evidence. The file witness remains single-process component infrastructure, not production storage. Production witness/storage, Valkey provenance/HA/failover, production ACL separation, configuration/advertisement/deployment, aggregate, multi-controller, hostile multi-tenant, and production-readiness results remain absent |
...
| P4 Browser witnessed action-history component | ADR 0034; witnessed v2 implementation; pinned Valkey index `sha256:ccfa19b0d743e48927e1c8c14e39e0acb97b5cea347fef0bfe340247fea920cd`; unexecuted 18-scenario v2 runner | The explicit v2 successor adds permanent bounded fingerprint-only history, strict whole-hash validation, an independent monotonic checkpoint witness, exact one-step interrupted-CAS recovery, and stable fail-closed errors. Focused/full race-shuffle, vet, Contract verification, the unchanged 48-case Suite, and tagged real-Valkey adapter integration pass. It does not rewrite v1 or alter default composition. A separately locked real-Chromium caller harness is implemented but has no clean local or hosted result. The included file witness is local single-process infrastructure, not HA or production storage. No v2 external-caller deletion/restore, production witness, operational restore, deployment, or production gate is closed |
...
| P4 browser Contract authority and reference callers | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; downstream component `b4d41c9`; ADR 0034 component and unexecuted runner; local downstream harness/run `550c785`/`20260906T050213.016063000Z`; hosted checkout/downstream run `2cadc53`/`34013982796` | Browser authority and the Browser reference, shared-capacity, durable-revocation, downstream-fencing, and witnessed-v2 tracks remain separate. The downstream track passes its 13-scenario v1 caller gate on local arm64 and hosted amd64; its 48-case Suite remains unexercised. ADR 0034 passes local deletion/rollback-detection component and pinned-Valkey adapter gates and now has a locked v2 caller runner, but no clean runner result. V2 caller/Chromium restore evidence, production witness/storage, Valkey provenance/HA/failover, production configuration/metrics/deployment and topology, real platform, aggregate, multi-controller, hostile multi-tenant, and production evidence remain absent |
...
- ADR 0034 adds an explicitly selected v2 successor without changing those v1
  ...
  Suite, and tagged integration against the pinned Valkey index pass. This is
  component and real-backend adapter evidence only. A separately locked
  ...
  fault identity, but it has no clean local or hosted run. It therefore adds no
  Chromium deletion/restore caller, production witness, HA/failover,
  production ACL, deployment, or production evidence.
...
- execution and inspection of the locked v2 multi-process real-Chromium caller
  gate covering retained-history deletion, controlled older-snapshot restore,
  witness reconstruction, and no upstream dial/old action; Browser production
...
Next, commit, execute, and inspect the locked ADR 0034 v2 multi-process
real-Chromium caller gate, including retained-history deletion, controlled
older-snapshot restore, independent witness reconstruction, and proof of no
upstream dial or old action.
