...
| --- | --- | --- |
| Latest Provider implementation evidence | Contract `5096e71`; projection/lock `24b2e36`; browser session `9a5d225`; sandbox/provenance implementation `6e02f1c`; Browser adapter `cd33ba3`; Browser provenance verifier `9390554`; Browser restricted-egress/create-policy `7e60340`; protected transport `b8423f5`; caller-owned Gateway `5aae281`; default-disabled command graph `66183b1`; DNS compatibility Provider `f760369`; Browser Gateway local capacity `7b062e6`; Browser pre-upgrade edge `44ea2ee`; Browser public TLS edge `b8f8941`; authenticated capacity `997fb0d`; E2E lock `49d1c20`; preceding hosted E2E harness `7a20d9d` | Locks Browser authority, composes the default-disabled Provider-local graph, bounds process-local Gateway/pre-upgrade/listener work, and adds a required post-binding authenticated-capacity port with typed lease-loss handling plus a process-local global/tenant/session memory reference. Local component, Contract, 48-case, and co-located lock/regression gates pass. Hosted evidence for this new lock has not yet been obtained. No real shared adapter, cross-process coordination, production advertisement, Provider-owned public Gateway, distributed durable revocation, production storage/configuration and metrics, real Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production claim is made |
...
| Browser-lock reference regression evidence | hosted harness/Provider `7a20d9d`/`b8f8941`, run `33857739105`; latest local run `20260904T081521.464863000Z` uses the earlier `249cdd4`/`44ea2ee` lock | Hosted and local Reference E2E each passed all 15 initial and 5 process-reconstruction/resume coding/shell scenarios. Hosted artifact `reference-e2e-evidence-33857739105` has digest `sha256:b9cbf768e7d7b1241643f309cd08cef69efe8c7d9b3a464c3da953f9d03adbb7`; the local run pins runtime digest `sha256:bcd5dbff8b2d108ee7dab464a85ee7d39ef74a8616a6af73a94ebb10ff8eaf75`. This is coding/shell regression evidence, not Browser E2E |
| Browser-lock Agent Platform candidate regression evidence | hosted harness/Provider `7a20d9d`/`b8f8941`, run `33857739189`; latest local run `20260904T081706.648122000Z` uses the earlier `249cdd4`/`44ea2ee` lock | Hosted and local candidate E2E each passed all 15 initial plus 5 resume coding/shell scenarios, including candidate shadow/selection/rollback/drain policy and reconstruction. Hosted artifact `platform-candidate-e2e-evidence-33857739189` has digest `sha256:61cf80c48b7103418f66acb0e370f19b60b2685271f2bb4a15f68547e4041bbb`; the local run pins runtime digest `sha256:bcd5dbff8b2d108ee7dab464a85ee7d39ef74a8616a6af73a94ebb10ff8eaf75`. This is candidate integration only, not Browser evidence, real Veronica, or production evidence |
...
| P4 optional-profile authority | Contract `5096e71`; projection/lock `24b2e36`; 48-case Suite; command graph `66183b1`; Browser capacity `7b062e6`; Browser edge `44ea2ee`; Browser TLS edge `b8f8941`; authenticated capacity `997fb0d`; E2E lock `49d1c20`; preceding Browser caller harness `7a20d9d` | Browser-only capability, create/session/handoff, opaque reference, usage meter, security matrix, admission, default-disabled command composition, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 listener/TLS/HTTP caller evidence, and authenticated-capacity port/memory component evidence have named boundaries. Production advertisement/public Gateway, a real shared adapter and two-Gateway-process evidence, distributed durable revocation, production storage/configuration and metrics, remaining profile-specific security/concurrency, aggregate, multi-controller, tenant, deployment, and production gates remain open |
...
| P4 Browser authenticated-capacity port and memory component | ADR 0030; implementation `997fb0d`; E2E lock `49d1c20` | Browser composition now requires a caller-owned capacity authority after exact grant binding and before revocation, authorized audit, Provider resolution, or dial. Typed loss/unavailability terminates both streams without reconnect; release uses an independent bounded context; simultaneous causes resolve as revocation, expiry, capacity, then transport. The memory reference atomically enforces global, tenant, and exact tenant/sandbox/session limits without storing caller, grant, credential, handoff, endpoint, or backend identity. Focused/full race-shuffle, vet, unchanged Contract verification and 48-case Suite pass. The independent E2E module race/shuffle, vet, and three lock checks pass against Provider `997fb0d`. Hosted CI for this lock is pending. This is port, process-local memory-component, and co-located lock/regression evidence only; no shared-store TTL/renewal, ownership, crash reclamation, stale-owner fencing, two-Gateway-process, distributed revocation, deployment, or production evidence is claimed |
...
| P3 candidate integration evidence | hosted harness/Provider `7a20d9d`/`b8f8941`, run `33857739189`; latest local evidence `e2e/evidence/platform-candidate/20260904T081706.648122000Z` uses the earlier `249cdd4`/`44ea2ee` lock | Hosted and local candidate regressions passed 15+5 coding/shell scenarios against Contract 48 cases. Hosted artifact digest is `sha256:61cf80c48b7103418f66acb0e370f19b60b2685271f2bb4a15f68547e4041bbb`; local runtime digest is `sha256:bcd5dbff8b2d108ee7dab464a85ee7d39ef74a8616a6af73a94ebb10ff8eaf75`. Boundary is `Agent Platform candidate integration only`; real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | hosted harness/Provider `7a20d9d`/`b8f8941`, run `33857739105`; latest local evidence `20260904T081521.464863000Z` uses the earlier `249cdd4`/`44ea2ee` lock | Hosted and local runs passed 15 initial plus 5 restart/resume coding/shell scenarios. Hosted artifact digest is `sha256:b9cbf768e7d7b1241643f309cd08cef69efe8c7d9b3a464c3da953f9d03adbb7`; local runtime digest is `sha256:bcd5dbff8b2d108ee7dab464a85ee7d39ef74a8616a6af73a94ebb10ff8eaf75`. It contains no Browser scenario and proves no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production property |
| P4 browser Contract authority and reference caller | Contract `5096e71`; projection/lock `24b2e36`; command `66183b1`; capacity `7b062e6`; service edge `44ea2ee`; TLS edge `b8f8941`; authenticated capacity `997fb0d`; current E2E lock `49d1c20`; preceding hosted harness/Provider `7a20d9d`/`b8f8941`; Browser run `33857739150` | Browser authority, DTO projection, image/runtime components, default-disabled command graph, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 listener/TLS/HTTP scenarios, and authenticated-capacity port/memory component gates pass within their names. The preceding hosted artifact digest is `sha256:94bcdfa53b667d4a6bc17fd6714cc9895e8402b830b8c6425c604c835f9228f`; it predates ADR 0030. A real shared adapter, two-Gateway-process run, distributed durable revocation, production storage/configuration and metrics, production advertisement/public Gateway, real platform, aggregate, multi-controller, hostile multi-tenant, deployment, and production evidence remain absent |
...
| P2.5i | Independent reference caller release gate | Hosted run `33857739105` with harness/Provider `7a20d9d`/`b8f8941` passed 15 initial and 5 restart/resume coding/shell scenarios; latest local run `20260904T081521.464863000Z` passes the same set on the earlier `249cdd4`/`44ea2ee` lock | Hosted artifact digest is `sha256:b9cbf768e7d7b1241643f309cd08cef69efe8c7d9b3a464c3da953f9d03adbb7`; the local run pins runtime digest `sha256:bcd5dbff8b2d108ee7dab464a85ee7d39ef74a8616a6af73a94ebb10ff8eaf75`. Neither run contains a Browser scenario and neither may be promoted to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P4 | Optional capability profiles | Browser Contract authority/projection, exact sandboxed signed linux/amd64 plus linux/arm64/v8 publication, Provider-local components, default-disabled command graph, process-local Gateway/pre-upgrade/listener limits, hosted 13+5 listener/TLS/HTTP Browser caller evidence, and ADR 0030 authenticated-capacity port/memory component evidence have named boundaries | Implement a real shared adapter and pass the two-independent-Gateway TTL/renewal/crash/fencing/loss/recovery gate; then close distributed durable revocation, production storage/configuration and metrics, remaining hostile-tenant/operational, and deployment gates before any production advertisement or Desktop authority audit |
...
- P4 Browser authenticated-capacity verification covers explicit dependency
  ...
  lock checks. Hosted CI and caller runs for this lock remain pending. A real
...
- Browser production advertisement and deployable public caller-owned Gateway;
  a real shared capacity adapter plus two-independent-Gateway evidence for
  atomic partitions, TTL/renewal, crash reclamation, stale-owner fencing,
  lease-loss termination, unavailability closure, and recovery; durable
  distributed revocation; production storage/configuration and metrics; and
...
listener/TLS/HTTP evidence. Authenticated partition-aware shared or distributed
capacity, durable distributed revocation, production storage/configuration and metrics,
...
Provider revision; their lock/regression checks pass locally. Hosted CI and
caller evidence for this lock are pending. The next implementation slice is a
reviewed real shared adapter plus a black-box run with two independently
started Gateway processes proving atomic global/tenant/session enforcement,
TTL/renewal, crash reclamation, stale-owner fencing, lease-loss termination,
renew/release races, unavailable-store failure closure, and recovery. Until
then the shared-capacity gate remains open.
