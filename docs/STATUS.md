...
| --- | --- | --- |
| Latest Provider implementation evidence | implementation `e5d7324ef1d4508b8b0c474fe5ead47edd6f5146` | Adds the internal Block manifest foundation without changing Provider Contract behavior. Provider behavior remains based on `d58497e5359056858564b9ac663178958cf5a6d6`; Block component tests and the current full local verification pass. No new Provider route, capability advertisement, or production claim is implied |
| Latest repository CI evidence | hosted workflow baseline `c7ff5eba6609bb0d047ab6d4cb1f6c53b09d27ed` | Repository CI run `33460370563` passed `provider-contract`, `test`, and `docker-integration`; the run emitted only GitHub's Node.js 20 deprecation warning for pinned Actions. Earlier P2.5h and P2.5g implementation gates remain in runs `33159099578` and `33157119149`. These are repository CI results only, not external-caller or production evidence |
| Latest reference external-caller evidence | co-located `e2e/` harness `2f165f9eb023392c1b6fb33845549f27e7364734`; local run `20260902T035608.264731000Z` | Against Provider `e5d7324`, all 15 initial and 5 process-reconstruction/resume scenarios passed over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. The run manifest pins Contract 38 cases and a linux/amd64 named runtime digest. Hosted run `33460370593` remains the latest published artifact. This is P2.5i reference caller evidence only |
| Latest Agent Platform candidate integration evidence | co-located candidate harness `2f165f9eb023392c1b6fb33845549f27e7364734`; local run `20260902T035641.384559000Z` | The independently named candidate caller passed lock verification and all 15 initial plus 5 resume scenarios against Provider `e5d7324`, including live capability/request shadow checks, ProviderRevision selection, canary rollback/drain policy, state reconstruction, mTLS/JWS, WebSocket, and Docker. Hosted run `33460370618` remains the latest published candidate artifact. This is candidate integration evidence only, not real Veronica or production evidence |
...
| P4 optional-profile authority | ADR 0017 and plan `p4-optional-profiles.md` | Browser-first Contract/image/security planning and the 2026-09-02 locked-resource gap inventory are recorded. No optional profile, Provider Contract resource, route, runtime image, or capability advertisement has been added |
...
| P3 candidate integration evidence | implementation `2f165f9`; local evidence `e2e/evidence/20260902T035641.384559000Z`; hosted baseline `c7ff5eb`, run `33460370618` | Local and Hosted candidate capability/request shadow checks, ProviderRevision binding, canary rollback preserving old-run identity, new-run stable selection, candidate state reconstruction, 15 initial and 5 resume scenarios, candidate metrics, and black-box import boundary passed. Boundary is `Agent Platform candidate integration only`; real platform shadow parity, canary traffic, rollback/drain, and platform-contract ownership remain open |
| P2.5i reference external caller | co-located harness `2f165f9eb023392c1b6fb33845549f27e7364734`; Provider `e5d7324ef1d4508b8b0c474fe5ead47edd6f5146`; local run `20260902T035608.264731000Z`; hosted run `33379217800` | Clean local and hosted runs passed 15 initial plus 5 restart/resume scenarios, including two controller contexts, endpoint non-disclosure, negative authorization, expiry/revocation, retained evidence, and same-shell reconnect. The current local manifest pins the unchanged Contract identity and a named linux/amd64 runtime digest. No Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production claim |
...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `2f165f9` against Provider `e5d7324`; evidence `20260902T035608.264731000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, Docker, and a temporary-registry named digest | Hosted run `33379217800` remains the latest published reference artifact and passed the same 15 initial and 5 restart/resume scenarios. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
| P3 | Migration readiness and platform integration | Local component evidence (`4212e88`) plus local candidate integration (`20260902T035641.384559000Z`, 15+5) and Hosted candidate integration (`47bd627`/`c7ff5eb`) are present; the reference P2 caller gate passes | Supply a real platform migration target and prove locked-Suite/request shadow parity, new-run-only canary, rollback, old-run drain, metric parity, and unchanged platform-owned contracts |
...
| P4 | Optional capability profiles | Authority/readiness planning and browser gap inventory recorded; implementation not started | Add browser, desktop, port forwarding, snapshots/restore, GPU, nested-container, and stronger-isolation profiles only through independent Contract, security, and conformance gates |
...
- Internal Block manifest verification covers strict YAML/JSON decoding,
  ...
  38-case Suite, independent E2E module race/shuffle/vet/lock checks, and both
