...
Product v1 Phase 2 is implementation-complete as an uncommitted candidate.
The source candidate contains all nine planned deliverable families and a
60-case local Suite, but formal completion remains open: the exact Provider
lock and clean-VCS Suite require a committed immutable revision, and the
planned independent-process coding/shell lifecycle black-box gate has not yet
been run. The historical lock and external-caller evidence are unchanged and
remain authoritative for their recorded scope.
...
| Product v1 Phase 2 Provider lifecycle closure | [Fixed nine-step plan](plan/product-v1-phase-2-provider-lifecycle.md); [Step 1 audit](audits/phase-2-step-1-provider-lifecycle-authority-gap.md); [candidate completion](audits/phase-2-provider-lifecycle-completion.md); ADR 0048 | The worktree contains the coordinated Contract candidate, durable lifecycle mutations/events/expiry, exact-owned fake and Docker controls, terminal close/revocation/cleanup, protected transport, capability readiness, and release tests. Focused packages and tagged Docker integration pass. Formal selection remains pending because the exact lock and clean-VCS Suite require a committed immutable revision and the independent-process coding/shell lifecycle black-box gate remains open; no external-caller, multi-controller, deployment, hostile-multitenant, or production claim exists |
...
| Current Provider Contract authority | Revision `22ba6987ea5fbc37d53942720133c0acad199edd`; tree `c9a7054d7c8e7f4b6e32f38175ceedddc48c2d38`; manifest `sha256:1e17e0ef4f86e03be1dac22c48e7b556a8600a4baa6252f4514390d339b8ba3f`; 53-case local Suite `sha256:b40c932643f4a1e5fd6681e3abf9b64a607609866a6254456970f8b8034cf2a8`; unchanged 6-case remote Suite | At documentation baseline `27081dfde4dfc726ba4df06d1f3b50012a04d2f2`, core CI `35204434771` passed Contract/profile/protocol verification, the clean VCS-built current 53-case local Suite, DTO projections, full race tests, vet, Docker integration, browser provenance, and Ubuntu/macOS supervisor matrices. A fresh current six-case remote discovery run and the historical E2E parent-lock/eight-check bundle are not relabeled by this CI result |
...
| P2.6 | Content-derived and portable Provider conformance | Historical 50-case local and six-case remote profiles passed at `3fe314a`/`ae476fe`; the current clean VCS-built 53-case local Suite passed core CI `35204434771` | No fresh current remote/E2E bundle is claimed. Independent caller qualification is separately complete under P2.7; protected/mutating remote and broader gates remain open |
...
For the current 53-case authority, core CI `35204434771` passed the Contract and
profile/protocol verification, clean VCS-built local Suite, DTO projections,
full root race/shuffle tests, vet, Docker integration, Browser provenance, and
Ubuntu/macOS supervisor matrices. This does not relabel a fresh current
six-case remote discovery run or the historical parent-lock/eight-check E2E
bundle; those retain their recorded identities.
