...
| --- | --- | --- |
| Latest local implementation evidence | P2.5f4 implementation `14f14cc` | Bounded caller-admitted WebSocket and terminal-byte-stream adapters, a direct `github.com/coder/websocket@v1.8.15` dependency, and focused race/shuffle tests passed. Full race/shuffle, vet, module verification, unchanged Contract verification, locked 38-case Suite, both real Docker regression packages, and diff checks also passed in this implementation state. This is adapter/component evidence only; no Gateway composition, command composition, external caller, or readiness claim is made |
| Latest repository CI evidence | P2.5f3 implementation `b1acdd1`; evidence baseline `7138a4c` | Repository CI `33059304542` passed `provider-contract`, `test`, and `docker-integration` for P2.5f3. CI for P2.5f4 `14f14cc` has not yet been recorded. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f4 | WebSocket and terminal stream adapters | Local component and full regression gates passed in implementation `14f14cc`; f4 CI is pending | Compose f5 only with caller-owned authorization, revocation, and recorder ports. Retain no-command-composition/no-external-caller boundary |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1-f3 pass locally and in CI, and P2.5f4 local adapter evidence passes. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f5-h Provider vertical composition, record f4 CI separately, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  evidence also pass. P2.5f4 WebSocket and terminal stream adapter local
  evidence passes while its CI is pending; Gateway/command composition remains
  next;
