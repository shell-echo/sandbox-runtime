...
| P2.5a | Coding/shell vertical-composition authority and delivery plan | Passed as design evidence in ADR 0015 and `docs/plan/p2.5-coding-shell-vertical-composition.md`; no Contract, config, runtime, route, or advertisement behavior changed | P2.5b must replace the current zero-or-terminal-only Contract authority with an explicitly locked coding/shell profile before runtime composition |
