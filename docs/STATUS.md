...
| Latest reference external-caller evidence | co-located `e2e/` harness `b28923b25079245c3088993badb4831c44598494`; ignored evidence `20260831T025259.339167000Z` | Against Provider `d58497e`, all 15 initial and 5 process-reconstruction/resume scenarios passed over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. The manifest pins both commits, Contract revision/tree, 38-case Suite identity, configuration digests, and a local tag plus immutable image-ID digest. This is P2.5i reference caller evidence only; hosted rerun after the image-ID fix is pending |
...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `b28923b` against Provider `d58497e`; evidence `20260831T025259.339167000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, and Docker | Rerun the root `.github/workflows/reference-e2e.yml` after the local image-ID digest fix and retain its artifact. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
- successful execution and publication of the co-located, independently
  ...
  `.github/workflows/reference-e2e.yml`; hosted run `33351621726` failed because
  a locally built Docker image had no registry RepoDigest, and fix `b28923b`
  now falls back to an inspected immutable image-ID digest. The clean local
  15+5 scenario result is external-caller evidence, while the Provider command
  root correctly remains free of caller-owned authorization, revocation,
  recording, and public Gateway defaults;
...
| Input | Locked value |
| --- | --- |
| Caller harness | `b28923b25079245c3088993badb4831c44598494` |
...
| Evidence directory | `20260831T025259.339167000Z` under `e2e/evidence/` (ignored) |
