...
| --- | --- | --- |
| Latest verified implementation/evidence baseline | P2.5f3 implementation `b1acdd1`; P2.5f2 CI evidence baseline `3abefd4` | P2.5f3 full race/shuffle, vet, module and unchanged Contract verification, locked 38-case Suite, focused opaque-reference tests, both real Docker integration packages, and diff checks passed locally at `b1acdd1`. Repository CI is pending. The latest completed CI is P2.5f2 run `33045725476`, which passed `provider-contract`, `test`, and `docker-integration` for evidence baseline `3abefd4`. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f3 | Opaque reference registry and resolver | Passed local gate in implementation `b1acdd1`; repository CI pending | Implement P2.5f4 WebSocket and terminal stream adapters. Retain the separate non-atomic session/reference stores and no-command-composition boundary |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1-f2 pass locally and in CI, and P2.5f3 passes locally with CI pending. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f4-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  pass. P2.5f3 opaque reference persistence/resolution passes locally with CI
  pending; P2.5f4 WebSocket and terminal stream adapters are next;
