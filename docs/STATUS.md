...
| --- | --- | --- |
| Current local implementation evidence | `5917a57`; P2.5e exec vertical | Full race/shuffle, vet, Contract verification, locked 38-case Suite, focused exec/restart/cancellation tests, both real Docker integration packages, and diff checks passed locally. This commit is not pushed and has no CI evidence |
| Latest verified remote/CI implementation baseline | `e3a8265`; P2.5d implementation is `de18787` | P2.5d implementation and local evidence are included. Post-push CI `32929140044` passed `provider-contract`, `test`, and the expanded `docker-integration` job. Verify the live checkout independently; this evidence baseline is intentionally not a self-updating `HEAD` claim |
...
| P2.5e exec vertical local evidence | implementation commit `5917a57` | Protected exec/cancel/result/operation routes, strict preflight, durable accept-before-dispatch, independent exec persistence and migration, correlation/conflict checks, injected-clock ordering, retained terminal summary, restart/on-demand reconciliation, Docker execution/cancellation, private bounded capture, expiry, and unsupported-reference rejection passed focused/full race-shuffle, vet, unchanged Contract verification, the 38-case Suite, diff checks, and both live Docker integration packages. CI is pending; this is local single-controller Provider evidence only |
...
| P2.5e | Exec vertical composition | Passed locally in `5917a57`; CI pending | Retain exec admission/runtime/recovery matrices and empty advertisement. P2.5f terminal/Gateway is next; external caller, aggregate, reliability, tenancy, deployment, and production claims remain open |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-d gates pass locally and in CI, and P2.5e has local single-controller exec evidence. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2.5e local verification covers strict protected exec/cancel/result/operation
  ...
  CI has not run. Usage identity is bounded correlation only; a real usage
  collector remains P2.5g. This does not prove external caller compatibility,
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  and artifact/usage applications are not command-composed, the exec slice has
  no CI evidence, and startup advertises no capability or runtime profile;
