...
| Latest reference external-caller evidence | co-located `e2e/` harness `e9a99036cba2e09fa45118a0c34786bdd0b6d0e6`; ignored evidence `20260831T032610.353349000Z` | Against Provider `d58497e`, all 15 initial and 5 process-reconstruction/resume scenarios passed over mTLS/JWS HTTPS, WebSocket, separate caller/reference-stack processes, and Docker. The manifest pins both commits, Contract revision/tree, 38-case Suite identity, configuration digests, and a temporary-registry tag plus immutable named digest. This is P2.5i reference caller evidence only; hosted rerun after the temporary-registry fix is pending |
...
| P2.5i reference external caller | co-located harness `e9a99036cba2e09fa45118a0c34786bdd0b6d0e6`; Provider `d58497e5359056858564b9ac663178958cf5a6d6` | Clean local run passed 15 initial plus 5 restart/resume scenarios, including two controller contexts, endpoint non-disclosure, negative authorization, expiry/revocation, retained evidence, and same-shell reconnect. The harness is a separately runnable Go module and process boundary under `e2e/`; hosted runs `33351621726`, `33352179181`, and `33352689522` showed that local image IDs/RepoDigests are not a usable Provider image reference, so `e9a9903` now publishes through a temporary registry and selects a named digest; no Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production claim |
...
| P2.5i | Independent reference caller release gate | Passed locally with clean co-located harness `e9a9903` against Provider `d58497e`; evidence `20260831T032610.353349000Z` records 15 initial and 5 restart/resume passes, exact lock/config/image identities, separate processes, real sockets, Docker, and a temporary-registry named digest | Rerun the root `.github/workflows/reference-e2e.yml` after the temporary-registry fix and retain its artifact. Do not promote this reference result to Agent Platform, aggregate, multi-controller, hostile multi-tenant, deployment, or production evidence |
...
- successful execution and publication of the co-located, independently
  ...
  had no usable named `RepoDigest`. Harness `e9a9903` now starts a uniquely
  labeled temporary `registry:2`, pushes the local `linux/amd64` image, and
  selects the registry-backed `name@sha256:<manifest-digest>` reference. The
...
`.github/workflows/reference-e2e.yml`; hosted publication remains pending after
...
| Input | Locked value |
| --- | --- |
| Caller harness | `e9a99036cba2e09fa45118a0c34786bdd0b6d0e6` |
...
| Evidence directory | `20260831T032610.353349000Z` under `e2e/evidence/` (ignored) |
