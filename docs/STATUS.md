...
| --- | --- | --- |
| Sandbox Provider Calling Standard decision | ADR 0037; `contract/specification/provider-calling-standard-v1.md`; repository-owned Contract manifest | External consumers adapt to the exact locked `sandbox-runtime` Provider Contract. The former named Agent Platform P3 migration route is retired, while caller/Provider ownership separation and historical candidate evidence remain. This standard slice adds no capability or production-readiness claim. ADR 0038 now defines the generic issuer trust model; the current implementation is tracked separately below and must complete the coordinated slice validation before compatibility is claimed |
| ADR 0038 caller issuer trust implementation | ADR 0038; current `provider/admission`, startup configuration, Provider transport composition, Contract projection, and same-repository generic reference-caller changes | The current worktree implements one caller trust domain per protected listener: one explicit exact issuer with no default or fallback, Provider-local audience and revision anchors, issuer-scoped admitted URI SAN identities, and 1..32 frozen verification keys. Rotation overlaps distinct old/new `kid` values across a restart, stops old-key signing, waits 300 seconds, then removes the old key and restarts again. Issuer substitution remains an authentication failure; after signature verification, an exact mismatch with the Provider-local audience or revision is forbidden before mutation reservation. The full coordinated slice remains under validation. Same-repository generic reference-caller coverage is reference evidence only; it does not establish independently implemented external-caller interoperability. Multi-issuer admission, multi-tenant isolation, HA, deployment, production readiness, and independently implemented external-caller interoperability remain open |
...
| Contract lock | revision `96187ae3923ca0b741c3de1bc58f31dae9c57080`; tree `4f3c1f037efd505e1285068072369d257d68626d`; manifest `sha256:d91ba3fa883741ec65165d1071055c3fef9ec896ab88511de3923388f49a0934`; OpenAPI `sha256:e011c896904cc15dabf6039f9d7b1de73e441cf13c85eceb74af687a80b57da9`; semantic rules `sha256:138b5c65bdafa14eb5fe626c875fe5adc9270cb45c194c7d4cebb5d1f12d626b` | The Provider Calling Standard is now a manifest resource. Local verifier and 48-case Conformance pass. Suite file SHA-256 is `c9279cdebdfc49c1e52d123ec352501deb3925db27b7d20d09a38bf5002e205d`; the declared Suite digest remains the existing placeholder and is not presented as content-derived |
...
| P1.1e | ADR 0038 listener-local caller trust domain | Current implementation adds explicit no-default issuer configuration, Provider-local audience/revision anchors, a maximum 32-key frozen bundle, overlap rotation coverage, and generic same-repository caller configuration | Complete the coordinated Contract, projection, configuration, Provider, E2E, and lock validation. Do not claim multi-issuer, independently implemented external-caller interoperability, multi-tenant, HA, deployment, or production properties |
...
- complete the coordinated validation of the Sandbox Provider Calling Standard
  and ADR 0038 implementation across Contract, projection, startup
  configuration, Provider admission, active E2E locks, and the same-repository
  generic reference caller;
...
The immediate work is to complete the repository-owned Sandbox Provider Calling
Standard and ADR 0038 validation: verify the Contract and projection, startup
authority, Provider admission precedence, same-repository generic reference
caller, and refreshed active E2E locks without rewriting historical evidence.
That reference caller is not an independently implemented external platform.
The content-derived Suite digest, portable remote black-box runner,
multi-issuer design, and lifecycle-closure gaps remain later work before broad
third-party interoperability is claimed.
