...
| --- | --- | --- |
| Latest verified implementation/evidence baseline | P2.5f2 implementation `ccffd52`; repository CI pending | Full race/shuffle, vet, module and unchanged Contract verification, locked 38-case Suite, focused session coordination/restart/migration tests, both real Docker integration packages, and diff checks passed locally at `ccffd52`. The latest verified repository CI remains P2.5f1 run `33033284420` for evidence baseline `66dd3d1`. Verify the live checkout independently; these are intentionally not self-updating `HEAD` claims |
...
| P2.5f2 durable session coordination evidence | implementation commit `ccffd52`; repository CI pending | Session repository v2, valid v1 migration, provider-neutral allocation receipt/observation evidence, trusted lifecycle projection, session-local fencing high-water, durable accept-before-allocation, exact-identity lost-attachment recovery, no replacement after uncertain outcomes, restart reconciliation, expiry/cleanup retry, corruption rejection, and registry-supplied success evidence passed focused/full race-shuffle, vet, module/Contract verification, the locked 38-case Suite, diff checks, and both live Docker integration packages. No `ref:session:*` registry/resolver, WebSocket/Gateway or command composition, external caller, multi-controller, tenancy, deployment, or readiness evidence is claimed |
...
| P2.5f2 | Durable session coordination and repository migration | Passed local gate at implementation `ccffd52`; repository CI pending | Implement P2.5f3 opaque reference registry/resolver. Retain the single-controller, separate-repository, no-command-composition boundary |
| P2 | Coding/remote-shell profile | In progress: P2.1-P2.4a3 have bounded component/Contract evidence, P2.5a-e gates pass locally and in CI, P2.5f0 has design evidence, P2.5f1 passes locally and in CI, and P2.5f2 passes locally. The current command composition is not a complete coding/shell Provider, and no separately supplied caller is runnable | Complete P2.5f3-h Provider vertical composition, then supply an independently owned caller and reproducible P2.5i environment. Aggregate conformance, multi-controller reliability, multi-tenant security, deployment, and production gates remain open |
...
- P2.5f2 local verification covers session repository v2 and valid v1
  ...
  both live Docker integration packages passed at `ccffd52`. This does not add
  a `ref:session:*` registry/resolver, WebSocket adapter, Gateway or command
  composition, nonempty advertisement, external caller, aggregate conformance,
...
- P2 coding/shell Provider vertical composition. The command root can now
  ...
  and P2.5f2 durable coordination/migration local evidence pass; P2.5f2
  repository CI is pending and P2.5f3 opaque reference persistence/resolution
