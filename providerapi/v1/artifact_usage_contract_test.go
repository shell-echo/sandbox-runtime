...
func assertEvidenceReadFixture(t *testing.T, fixture evidenceReadMatrix) {
	t.Helper()
	wantOperationTypes := []string{"create", "extend_lease", "exec", "cancel_exec", "suspend", "resume", "terminate", "open_runtime_session", "close_runtime_session", "open_browser_session", "open_desktop_session", "close_desktop_session", "artifact_stage"}
...
func assertEvidenceReadOpenAPI(t *testing.T) {
	...
	if !strings.Contains(operationResponses["200"].Description, "artifact_stage") || !strings.Contains(operationResponses["200"].Description, "Browser-session") || !strings.Contains(operationResponses["200"].Description, "Desktop-session") || !strings.Contains(operationResponses["404"].Description, "any composed operation-family authority") {
