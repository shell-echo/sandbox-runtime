// Package file provides versioned, atomically replaced desktop-session state
...
func NewRepository(path string) (*Repository, error) {
	if strings.TrimSpace(path) == "" {
		return nil, errors.New("desktop session repository path is required")
	}
	cleanPath := filepath.Clean(path)
	if err := os.MkdirAll(filepath.Dir(cleanPath), 0o700); err != nil {
		return nil, fmt.Errorf("create desktop session repository directory: %w", err)
	}
	lockFile, err := acquireLock(cleanPath + ".lock")
	if err != nil {
		return nil, fmt.Errorf("lock desktop session repository: %w", err)
	}
...
func (r *Repository) ReserveClose(ctx context.Context, request desktop.CloseRequest, acceptedAt time.Time, expiration bool) (desktop.CloseReservation, error) {
	...
	err := r.mutate(ctx, func() error {
		var err error
		result, err = r.state.ReserveCloseAt(request, acceptedAt, expiration)
		return err
	})
	return result, err
}
...
func (r *Repository) UpdateClose(ctx context.Context, record desktop.CloseRecord, expected desktop.Status, source desktop.Record) error {
	return r.mutate(ctx, func() error { return r.state.UpdateClose(record, expected, source) })
}
func (r *Repository) ResolveUnknownClose(ctx context.Context, operationID string, observedAt time.Time) (desktop.Record, error) {
	var result desktop.Record
	err := r.mutate(ctx, func() error {
		var err error
		result, err = r.state.ResolveUnknownClose(operationID, observedAt)
		return err
	})
	return result, err
}
...
func (r *Repository) load() error {
	...
	if err != nil {
		return fmt.Errorf("open desktop session repository: %w", err)
	}
	defer file.Close()
	info, err := file.Stat()
...
func (r *Repository) persist(ctx context.Context) (bool, error) {
	...
	temporary, err := os.CreateTemp(directory, ".desktop-session-*.tmp")
	if err != nil {
