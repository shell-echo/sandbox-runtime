...
// signer identity. It does not prove restricted egress or Desktop composition.
func TestLockedPublicationIntegration(t *testing.T) {
	if os.Getenv("SANDBOX_RUNTIME_BROWSER_PROVENANCE_INTEGRATION") != "1" {
		t.Skip("set SANDBOX_RUNTIME_BROWSER_PROVENANCE_INTEGRATION=1 to verify the locked Desktop publication")
	}
	...
	defer cancel()
	if err := verifier.Verify(ctx, desktopimage.LockedPublication()); err != nil {
		t.Fatal(err)
	}
}
