...
func TestMobyEngineProjectsDesktopIsolation(t *testing.T) {
	...
	_, err = backend.create(context.Background(), createRequest{
		name: "desktop-runtime", image: "example.invalid/desktop@sha256:digest",
		labels: map[string]string{managedLabel: "true"}, user: DesktopUser, workingDirectory: "/workspace",
		...
		networkName: "desktop-egress-network-1", dnsResolver: "10.88.0.2",
	})
	if err != nil {
		t.Fatal(err)
	}
	if captured.Config == nil || captured.Config.User != DesktopUser || captured.Config.WorkingDir != "/workspace" ||
	...
	if host == nil || host.NetworkMode != "desktop-egress-network-1" || host.Privileged || host.AutoRemove ||
