...
func NewRegistry(path string) (*Registry, error) {
	if strings.TrimSpace(path) == "" {
		return nil, errors.New("desktop reference registry path is required")
	}
...
func (r *Registry) FindRunning(ctx context.Context, source desktop.Record) (reference.Record, error) {
	if err := contextError(ctx); err != nil {
		return reference.Record{}, err
	}
	r.mu.RLock()
...
func (r *Registry) persist(ctx context.Context) (bool, error) {
	...
	file, err := os.CreateTemp(directory, ".desktop-reference-*.tmp")
	if err != nil {
