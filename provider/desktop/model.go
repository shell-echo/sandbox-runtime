// Package desktop contains the backend-neutral Provider-local desktop session
...
var (
	ErrInvalidRequest           = errors.New("invalid Provider desktop session request")
	ErrDeadlineExpired          = errors.New("Provider desktop session deadline has expired")
	ErrInvalidExpiry            = errors.New("invalid Provider desktop session expiry")
	ErrInvalidRecord            = errors.New("invalid Provider desktop session record")
	ErrInvalidHandoff           = errors.New("invalid Provider desktop session handoff")
	ErrInvalidAllocation        = errors.New("invalid Provider desktop session allocation evidence")
	ErrAllocationConflict       = errors.New("Provider desktop session allocation evidence conflict")
	ErrInvalidTransition        = errors.New("invalid Provider desktop session transition")
	ErrOperationTerminal        = errors.New("Provider desktop session operation is terminal")
	ErrAuthorityNotFound        = errors.New("Provider desktop session authority record was not found")
	ErrHandoffUnavailable       = errors.New("Provider desktop session handoff is unavailable")
	ErrHandoffExpired           = errors.New("Provider desktop session handoff has expired")
	ErrHandoffRevoked           = errors.New("Provider desktop session handoff has been revoked")
	ErrSandboxNotReady          = errors.New("Provider desktop session sandbox is not ready")
	ErrProviderRevisionConflict = errors.New("Provider desktop session provider revision conflict")
	ErrGenerationConflict       = errors.New("Provider desktop session sandbox generation conflict")
	ErrLeaseExpired             = errors.New("Provider desktop session sandbox lease has expired")
	ErrStaleFencingToken        = errors.New("Provider desktop session fencing token is stale")
	ErrCapabilityUnsupported    = errors.New("Provider desktop session capability profile is unsupported")
	ErrNetworkPolicyConflict    = errors.New("Provider desktop session network policy conflict")
	ErrDesktopNotFound          = errors.New("Provider desktop allocation was not found")
	ErrDesktopExpired           = errors.New("Provider desktop allocation has expired")
	ErrDesktopConflict          = errors.New("Provider desktop allocation identity conflict")
	ErrDesktopCapacity          = errors.New("Provider desktop session capacity is exhausted")
	ErrDesktopUnsupported       = errors.New("Provider desktop runtime is unsupported")

	identifierPattern = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$`)
	digestPattern     = regexp.MustCompile(`^sha256:[0-9a-f]{64}$`)
	endpointPattern   = regexp.MustCompile(`^ref:desktop-session:[A-Za-z0-9][A-Za-z0-9._-]{0,199}$`)
	allocationPattern = regexp.MustCompile(`^ref:desktop/[0-9a-f]{32}$`)
)

const (
	CapabilityProfileID = "desktop-v1"
...
// OpenRequest is the provider-local projection of a desktop session request.
...
func (r AllocationReceipt) Validate() error {
	...
	for _, value := range []string{r.SandboxID, r.DesktopSessionID, r.OperationID, r.AttemptID} {
		if !identifierPattern.MatchString(value) {
			return ErrInvalidAllocation
		}
	}
...
func (r AllocationReceipt) matchesRequest(request OpenRequest) bool {
	return r.SandboxID == request.SandboxID && r.DesktopSessionID == request.DesktopSessionID &&
...
func (h Handoff) Validate() error {
	for _, value := range []string{h.OperationID, h.AttemptID, h.SandboxID, h.DesktopSessionID} {
		if !identifierPattern.MatchString(value) {
			return ErrInvalidHandoff
		}
	}
	if h.FencingToken < 1 || h.CapabilityProfileID != CapabilityProfileID || h.Protocol != ProtocolWebRTC ||
		h.MediaProfileID != MediaProfileID || h.ControlProfileID != ControlProfileID || h.ExpiresAt.IsZero() {
		return ErrInvalidHandoff
	}
...
type Record struct {
	...
	CloseUnknown bool                `json:"close_outcome_unknown,omitempty"`
}
...
func (r Record) Validate() error {
	...
	} else if r.Status == StatusRunning && r.Allocation != nil && r.Allocation.State != AllocationRunning {
		return ErrInvalidRecord
	}
	if (r.RevokedAt != nil || r.ClosedAt != nil || r.Expired || r.CloseUnknown) && r.Status != StatusSucceeded {
		return ErrInvalidRecord
	}
	if r.ClosedAt != nil && r.RevokedAt == nil || r.Expired && r.ClosedAt == nil || r.CloseUnknown && (r.RevokedAt == nil || r.ClosedAt != nil) {
		return ErrInvalidRecord
	}
	if r.RevokedAt != nil && (r.RevokedAt.IsZero() || r.RevokedAt.Before(r.ObservedAt)) {
		return ErrInvalidRecord
	}
	if r.ClosedAt != nil && (r.ClosedAt.IsZero() || r.ClosedAt.Before(*r.RevokedAt)) {
		return ErrInvalidRecord
	}
	if r.Allocation != nil {
...
func AttachAllocation(record Record, receipt AllocationReceipt) (Record, error) {
	...
	if record.Status != StatusAccepted && record.Status != StatusRunning && record.Status != StatusOutcomeUnknown {
...
func ObserveAllocation(record Record, observation AllocationEvidence) (Record, error) {
	...
	if record.Allocation == nil || record.Status != StatusRunning && record.Status != StatusOutcomeUnknown {
		return Record{}, ErrOperationTerminal
	}
...
var transitions = map[Status]map[Status]bool{
	StatusAccepted:       {StatusRunning: true, StatusFailed: true, StatusCancelled: true, StatusOutcomeUnknown: true},
	StatusRunning:        {StatusSucceeded: true, StatusFailed: true, StatusCancelled: true, StatusOutcomeUnknown: true},
	StatusOutcomeUnknown: {StatusRunning: true, StatusFailed: true, StatusSucceeded: true},
}
...
func Transition(record Record, next Status, observedAt time.Time, evidence *EndpointEvidence) (Record, error) {
	...
	updated.Handoff = &Handoff{
		...
		DesktopSessionID: record.Request.DesktopSessionID, CapabilityProfileID: CapabilityProfileID,
		Protocol: ProtocolWebRTC, MediaProfileID: MediaProfileID, ControlProfileID: ControlProfileID,
...
func handoffMatchesRequest(handoff Handoff, request OpenRequest) bool {
	return handoff.OperationID == request.OperationID && handoff.AttemptID == request.AttemptID &&
		...
		handoff.DesktopSessionID == request.DesktopSessionID && handoff.CapabilityProfileID == request.CapabilityProfileID &&
