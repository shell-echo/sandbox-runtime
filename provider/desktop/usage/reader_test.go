...
type sessionLister struct {
	records []desktop.Record
	err     error
}

func (l sessionLister) ListOpen(context.Context) ([]desktop.Record, error) {
	result := make([]desktop.Record, len(l.records))
...
func TestReaderProjectsPartialThenPersistsCompleteExpiryEvidence(t *testing.T) {
	...
	reader, err := NewReader(sessionLister{records: []desktop.Record{record}}, store, time.Hour)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReaderPreservesMissingUnavailableAndExpiredStates(t *testing.T) {
	...
	pending.Status = desktop.StatusRunning
	pending.Handoff = nil
	reader, _ = NewReader(sessionLister{records: []desktop.Record{pending}}, store, time.Hour)
	...
	reader, _ = NewReader(sessionLister{records: []desktop.Record{record}}, store, time.Hour)
