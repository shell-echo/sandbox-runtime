...
type SessionLister interface {
	ListOpen(context.Context) ([]desktop.Record, error)
}

// Reader derives Desktop duration evidence from the durable session record.
...
func (r *Reader) GetEvidence(ctx context.Context, operationID string, now time.Time) (providerusage.Evidence, error) {
	...
	if record.Status != desktop.StatusSucceeded || record.Handoff == nil {
		return providerusage.Evidence{}, providerusage.ErrEvidenceUnavailable
	}
