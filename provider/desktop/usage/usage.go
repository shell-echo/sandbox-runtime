// Package usage projects bounded desktop-session duration evidence. It is a
...
var (
	ErrInvalid     = errors.New("invalid desktop usage request")
	ErrUnavailable = errors.New("desktop usage evidence is unavailable")
)
...
func BuildEvidence(record desktop.Record, now, retainedUntil time.Time) (providerusage.Evidence, error) {
	return BuildEvidenceWithStops(record, now, StopTimes{}, retainedUntil)
}

func BuildEvidenceWithStops(record desktop.Record, now time.Time, stops StopTimes, retainedUntil time.Time) (providerusage.Evidence, error) {
	if err := record.Validate(); err != nil || record.Status != desktop.StatusSucceeded || record.Handoff == nil || record.Allocation == nil || record.Allocation.State != desktop.AllocationRunning {
	...
	for _, endedAt := range []*time.Time{record.RevokedAt, record.ClosedAt, stops.EndpointTerminatedAt, stops.SandboxTerminatedAt} {
	...
	if record.RevokedAt != nil && !record.RevokedAt.After(stopAt) {
		stopAt = record.RevokedAt.UTC()
		complete = true
	}
	if record.ClosedAt != nil && !record.ClosedAt.After(stopAt) {
		stopAt = record.ClosedAt.UTC()
		complete = true
	}
	...
	reference := "ref:usage/desktop-" + key
	evidence := providerusage.Evidence{EvidenceID: "desktop-usage-" + key, SandboxID: record.Request.SandboxID, OperationID: record.Request.OperationID, AttemptID: record.Request.AttemptID, FencingToken: record.Request.FencingToken, Entries: []providerusage.Entry{{EntryID: "desktop-duration-" + key, SandboxID: record.Request.SandboxID, OperationID: record.Request.OperationID, Meter: providerusage.MeterDesktopSession, Quantity: stopAt.Sub(record.ObservedAt).Milliseconds(), Unit: "milliseconds", MeterSource: providerusage.SourceRuntime, EvidenceReference: reference, OccurredAt: stopAt}}, ReconciliationStatus: status, ObservedAt: stopAt, RetainedUntil: retainedUntil}
...
func (c Collector) Collect(ctx context.Context, record desktop.Record) (providerusage.Evidence, error) {
	return c.CollectWithStops(ctx, record, StopTimes{})
}

func (c Collector) CollectWithStops(ctx context.Context, record desktop.Record, stops StopTimes) (providerusage.Evidence, error) {
