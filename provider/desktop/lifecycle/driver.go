// Package lifecycle provides the Desktop profile's Provider-local sandbox
// readiness adapter. A Desktop sandbox is a durable control-plane authority;
// the concrete Chromium allocation is created by the separate Desktop session
...
var (
	ErrInvalidDriver = errors.New("invalid Provider Desktop lifecycle driver")
	ErrUnavailable   = errors.New("Provider Desktop lifecycle readiness is unavailable")
)
...
func (d *Driver) Create(ctx context.Context, sandbox providerlifecycle.Sandbox) error {
	...
	if d == nil || nilDependency(d.runtime) || sandbox.Validate() != nil ||
		sandbox.RuntimeProfile != providerlifecycle.DesktopRuntimeProfile ||
