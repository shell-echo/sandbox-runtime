// Package docker provisions a fail-closed restricted-egress Docker network
// for one sealed Provider-local Browser or Desktop workload identity.
package docker
...
const (
	...
	// UplinkRole is the locked ownership label on the shared restricted-egress
	// uplink. Its historical value is retained for Browser compatibility.
...
var (
	ErrInvalidOptions     = errors.New("invalid restricted-network options")
	ErrPolicyUnavailable  = errors.New("egress policy is unavailable")
	ErrOwnershipConflict  = errors.New("restricted-network ownership conflict")
	ErrNetworkUnavailable = errors.New("restricted network is unavailable")
	ErrOutcomeUnknown     = errors.New("restricted-network outcome is unknown")
...
func (o Options) validate() (map[string]restricted.Policy, error) {
	...
	policies := make(map[string]restricted.Policy, len(o.Policies))
	for _, configured := range o.Policies {
		policy, err := restricted.NormalizePolicy(configured)
