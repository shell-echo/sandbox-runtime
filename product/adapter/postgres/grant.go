...
func (r *GrantRepository) MintConnectionGrant(ctx context.Context, command product.ConnectionGrantCommand) (product.ConnectionGrant, bool, error) {
	...
	return product.ConnectionGrant{ID: command.ConnectionID, SessionID: command.SessionID, ProtocolProfile: profile, GatewayURI: command.GatewayURI, Ticket: command.Ticket, ExpiresAt: expires}, false, nil
}
...
func (r *GrantRepository) ConsumeConnectionGrant(ctx context.Context, ticket string) (product.GatewayBinding, error) {
	...
	var now time.Time
	err = tx.QueryRow(opCtx, `SELECT g.connection_id,g.tenant_id,g.actor_type,g.actor_id,g.workspace_id,g.slot_key,g.slot_generation,g.session_id,g.protocol_profile,g.expires_at,g.state,COALESCE(g.control_lease_id,''),COALESCE(g.control_fence,0),s.state,b.provider_revision_id,b.sandbox_id,COALESCE(s.provider_handoff_reference,''),COALESCE(s.provider_connection_generation,0),s.provider_handoff_expires_at,clock_timestamp() FROM sandbox_runtime_product.connection_grants g JOIN sandbox_runtime_product.runtime_sessions s ON s.tenant_id=g.tenant_id AND s.session_id=g.session_id JOIN sandbox_runtime_product.provider_bindings b ON b.tenant_id=g.tenant_id AND b.workspace_id=g.workspace_id AND b.slot_key=g.slot_key AND b.slot_generation=g.slot_generation AND b.current WHERE g.ticket_digest=$1 FOR UPDATE OF g`, digest[:]).Scan(&binding.ConnectionID, &binding.TenantID, &actorType, &binding.Actor.ID, &binding.WorkspaceID, &binding.SlotKey, &binding.SlotGeneration, &binding.SessionID, &binding.ProtocolProfile, &binding.ExpiresAt, &grantState, &controlLeaseID, &controlFence, &state, &binding.ProviderRevisionID, &binding.SandboxID, &binding.HandoffReference, &binding.ConnectionGeneration, &binding.HandoffExpiresAt, &now)
...
func (r *GrantRepository) CheckGatewayAuthority(ctx context.Context, binding product.GatewayBinding) error {
	...
	if binding.ControlLeaseID != "" {
		err = r.store.pool.QueryRow(opCtx, `SELECT EXISTS(SELECT 1 FROM sandbox_runtime_product.control_leases WHERE tenant_id=$1 AND lease_id=$2 AND state='active' AND fence=$3 AND expires_at>clock_timestamp())`, binding.TenantID, binding.ControlLeaseID, binding.ControlFence).Scan(&valid)
		if err != nil {
			return product.ErrStoreUnavailable
		}
		if !valid {
...
func (r *GrantRepository) loadRetainedGrant(ctx context.Context, tx pgx.Tx, command product.ConnectionGrantCommand) (product.ConnectionGrant, error) {
	...
	var ciphertext []byte
	var keyID string
	if err := tx.QueryRow(ctx, `SELECT connection_id,session_id,protocol_profile,ticket_ciphertext,ticket_key_id,expires_at FROM sandbox_runtime_product.connection_grants WHERE tenant_id=$1 AND connection_id=$2`, command.TenantID, connectionID).Scan(&grant.ID, &grant.SessionID, &grant.ProtocolProfile, &ciphertext, &keyID, &grant.ExpiresAt); err != nil {
		return product.ConnectionGrant{}, product.ErrStoreUnavailable
	}
	if keyID != r.keyID {
		return product.ConnectionGrant{}, product.ErrStoreUnavailable
