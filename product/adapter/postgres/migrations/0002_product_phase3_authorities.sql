...
CREATE TABLE sandbox_runtime_product.runtime_sessions (
    ...
    provider_handoff_reference text,
    provider_handoff_expires_at timestamp with time zone,
    expires_at timestamp with time zone NOT NULL,
    created_at timestamp with time zone NOT NULL,
    updated_at timestamp with time zone NOT NULL,
...
CREATE TABLE sandbox_runtime_product.connection_grants (
    ...
    ticket_ciphertext bytea NOT NULL,
    ...
    CONSTRAINT connection_grants_ticket_ciphertext CHECK (octet_length(ticket_ciphertext) >= 29),
    CONSTRAINT connection_grants_ticket_key CHECK (ticket_key_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$'),
