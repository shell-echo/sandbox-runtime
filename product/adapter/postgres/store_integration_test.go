...
func cleanupProductTenant(t *testing.T, pool *pgxpool.Pool, tenantID string) {
	...
	cleanup := func() {
		...
		for _, statement := range []string{
			...
			`DELETE FROM sandbox_runtime_product.development_environments WHERE tenant_id = $1`,
			`DELETE FROM sandbox_runtime_product.workspace_heads WHERE tenant_id = $1`,
			`DELETE FROM sandbox_runtime_product.workspace_revision_manifests WHERE tenant_id = $1`,
