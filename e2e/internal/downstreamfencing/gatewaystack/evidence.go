...
func readEvidenceSequence(file *os.File) (uint64, error) {
	...
	for scanner.Scan() {
		if strings.TrimSpace(scanner.Text()) == "" {
			return 0, errors.New("audit contains an empty record")
		}
		var record struct {
			Sequence uint64 `json:"sequence"`
		}
		if err := json.Unmarshal(scanner.Bytes(), &record); err != nil || record.Sequence != last+1 {
			return 0, errors.New("audit sequence is invalid")
		}
...
func (r *auditRecorder) Record(ctx context.Context, event gateway.AuditEvent) error {
	...
	return r.writer.append(func(sequence uint64) any {
		return auditRecord{
			...
			Attempt: event.Attempt, Frames: event.Frames, Bytes: event.Bytes, ReasonCode: reasonCode,
		}
	})
}
...
func auditReasonCode(eventType gateway.AuditEventType) (string, bool) {
	...
	case gateway.AuditRevocationUnavailable:
		return "revocation_unavailable", true
	case gateway.AuditDownstreamFenceLost:
		return "downstream_fence_lost", true
	case gateway.AuditDownstreamUnavailable:
		return "downstream_fence_unavailable", true
	default:
		return "", false
	}
}

var _ gateway.Recorder = (*auditRecorder)(nil)
