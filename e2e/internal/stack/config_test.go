...
func TestConfigValidateRejectsAmbiguousIdentityInputs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "missing issuer", mutate: func(config *Config) { config.JWSIssuer = "" }},
		{name: "legacy issuer", mutate: func(config *Config) { config.JWSIssuer = "agent-platform" }},
		{name: "fragment issuer", mutate: func(config *Config) { config.JWSIssuer += "#fragment" }},
