...
func TestTwoDownstreamCallerProcessesBootstrapIndependentlyAndRemainControllable(t *testing.T) {
	...
	providerA, callerConfigA, providerConfigA := newDownstreamProcessBootstrapFixture(t, "a", 11)
	providerB, callerConfigB, providerConfigB := newDownstreamProcessBootstrapFixture(t, "b", 21)
	processA := startDownstreamCallerProcess(t, binary, moduleRoot, callerConfigA, providerConfigA)
	processB := startDownstreamCallerProcess(t, binary, moduleRoot, callerConfigB, providerConfigB)
...
func newDownstreamProcessBootstrapFixture(t *testing.T, suffix string, createFence int64) (*browserBootstrapProvider, string, string) {
	t.Helper()
	...
	return provider, callerConfigPath, providerConfig
}
...
func startDownstreamCallerProcess(t *testing.T, binary, moduleRoot, callerConfig, providerConfig string) *downstreamCallerProcess {
	t.Helper()
