...
// Run processes strict JSONL controls until EOF or a successful shutdown.
func Run(ctx context.Context, config wire.RevokerConfig, in io.Reader, out io.Writer) error {
	if ctx == nil || in == nil || out == nil {
		return errors.New("revoker initialization failed")
	}
	revoker, err := New(ctx, config)
	if err != nil {
		return errors.New("revoker initialization failed")
	}
	defer revoker.Close()
	...
	for scanner.Scan() {
		...
		if !ok {
			...
			response = revoker.Execute(ctx, command)
		}
		if err := encoder.Encode(response); err != nil {
			return errors.New("revoker output failed")
		}
	...
	if err := scanner.Err(); err != nil {
		return errors.New("revoker input failed")
	}
	return nil
}
