...
func Open(ctx context.Context, input wire.GatewayConfig) (_ *Stack, resultErr error) {
	...
	operationTimeout := durationMillis(config.RevocationPolicy.OperationTimeoutMillis)
	client, err := newRedisClient(config.RedisURL, operationTimeout)
	if err != nil {
		return nil, err
	}
	stack.client = client
	revocations, err := redisrevocation.New(redisrevocation.Options{
		...
		MaxGrantLifetime: durationMillis(config.RevocationPolicy.MaxGrantLifetimeMillis),
		PollInterval:     durationMillis(config.RevocationPolicy.PollIntervalMillis),
		OperationTimeout: operationTimeout,
	})
	if err != nil {
		return nil, fmt.Errorf("construct durable revocation source: %w", err)
	}
	if err := revocations.Verify(ctx); err != nil {
		return nil, fmt.Errorf("verify durable revocation policy: %w", err)
	}
	capacity, err := gateway.NewLocalConnectionCapacity(gateway.LocalConnectionCapacityOptions{
		MaxTotal: config.LocalCapacity.MaxTotal, MaxPerTenant: config.LocalCapacity.MaxPerTenant,
		MaxPerSession: config.LocalCapacity.MaxPerSession,
	})
	if err != nil {
		return nil, fmt.Errorf("construct local authenticated capacity: %w", err)
	}
	...
	controller, err := newController(config.Principals, config.Endpoints, config.GrantBindings,
		durationMillis(config.RevocationPolicy.MaxGrantLifetimeMillis))
	if err != nil {
		return nil, err
	}
	...
	service, err := gatewaycomposition.NewBrowser(gatewaycomposition.BrowserOptions{
		Authorizer: controller, Revocations: revocations,
		...
		MaxReconnects:            config.ReconnectPolicy.MaxReconnects,
		ReconnectBackoff:         durationMillisAllowZero(config.ReconnectPolicy.BackoffMillis),
		CapacityReleaseTimeout:   gateway.DefaultCapacityReleaseTimeout,
		MaxConnections:           config.LocalCapacity.MaxTotal,
		MaxConnectionsPerSession: config.LocalCapacity.MaxPerSession,
	})
	if err != nil {
...
func (s *Stack) Run(ctx context.Context) error {
	if s == nil || s.server == nil || ctx == nil {
		return errors.New("durable-revocation Gateway is unavailable")
	}
...
func newController(principals []wire.Principal, endpoints []wire.Endpoint, bindings []wire.GrantBinding, maxGrantLifetime time.Duration) (*controller, error) {
	result := &controller{
		...
		maxGrantLifetime:  maxGrantLifetime,
	}
...
func (c *controller) Authorize(ctx context.Context, request gateway.ConnectRequest) (gateway.Grant, error) {
	...
	if !principalOK || !inputOK || !bindingOK || !endpointOK || input.request != request ||
		...
		input.expiresAt.After(now.Add(c.maxGrantLifetime)) {
